...
### 1. φ标签模式
**系数递归**：$a_0 = 0, a_1 = 1, a_2 = 1$，后续$a_k = a_{k-1} + a_{k-2}$（Fibonacci递归）
**模式函数**：$F_\phi = F_{\text{ratio}}(\{a_k\}) = \lim \frac{a_n}{a_{n-1}} = \phi$
**相对指标**：$\eta^{(\phi)}(k; m) = \frac{a_{m+k}}{a_m}$（$m \geq 1$或$a_m \neq 0$），对$m=0$情况通过Fibonacci性质的极限值处理
