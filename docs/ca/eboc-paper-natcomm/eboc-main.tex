...
\begin{abstract}
\textbf{Objective.} We present \textbf{EBOC (Eternal-Block Observer-Computing)}: a geometry--information unified framework without requiring explicit global time, merging the \textbf{timeless causal encoding} of \textbf{Eternal-Graph Cellular Automata} (EG-CA) with the \textbf{program semantics and observation-decoding} of \textbf{Static-Block Universe Cellular Automata} (SB-CA) into a single formal system, with verifiable information laws and construction algorithms. We regard the static block $X_f$ and the eternal-graph edge shift $(Y_G,\sigma)$ as \textbf{equivalent dual formulations}; below we use $X_f$ as the primary presentation, but every conclusion can be restated on $(Y_G,\sigma)$ via \textbf{graph-shift (countable-state Markov shift) display/encoding equivalence}; when satisfying T13's ``time-Markovizability'' hypotheses, one further obtains sofic/finite-type presentations.

\textbf{Three pillars.}
\begin{enumerate}
\item \textbf{Geometric encoding (Graph/SFT)}: universe as \textbf{static block} $X_f\subset\Sigma^{\Z^{d+1}}$ satisfying local rule $f$; its causality/consistency characterized in parallel by \textbf{eternal graph} $G=(V,E)$ and \textbf{subshift (SFT)}.
\item \textbf{Semantic emergence (Observation = Decoding)}: \textbf{observation = factor decoding}. Decoder $\pi:\Sigma^{B}\to\Gamma$ reads the static block \textbf{leaf-by-leaf} along acceptable foliations (cross-leaf reading from layer $c$ to $c+b$ along $\tau$), outputting visible language; ``semantic collapse'' is \textbf{information factorization} from base configuration to visible record.
\item \textbf{Information constraint (non-increase law)}: observation does not create information:
$$
K\big(\pi(x|_W)\big)\le K(x|_W)+K(\pi)+O(1),
$$
with condition-complexity upper bounds under \textbf{causal thick boundary} and Brudno-consistent entropy limits.
\end{enumerate}

\textbf{Unified metaphor (RPG game).} Universe like an \textbf{infinite-plot RPG}: \textbf{game data and evolution rules} already written ($(X_f,f)$); ``choice'' (apparent free will) must align with \textbf{plotline} (determinism). \textbf{Leaf-by-leaf reading} unlocks chapters at fixed rhythm $b$; ``choice'' is \textbf{representative selection} among compatible branches and \textbf{exclusion} of incompatible ones; the ontological ``ROM'' neither gains nor loses information.

\textbf{Core object.}
$$
\cU=(X_f, G, \rho, \Sigma, f, \pi, \mu, \nu),
$$
where $X_f$ is space-time SFT, $G$ eternal graph, $\rho$ specifies \textbf{acceptable leaf family} (level sets of primitive integral covector $\tau^\star$, $\langle\tau^\star,\tau\rangle=b\ge1$), $\pi$ decoder, $\mu$ shift-invariant ergodic measure, $\nu$ universal semimeasure (serving as typicality weight). Equivalently, all results can be expressed via $G$'s \textbf{edge shift} $Y_G$ and its \textbf{path shift} $(Y_G,\sigma)$; observation/information laws hold equally for both formulations.
\end{abstract}
...
\begin{table}[h]
\centering
\caption{Novelty classification of main results}\label{tab:novelty}
\small
\begin{tabular}{lll}
\hline
...
T2 & Refinement & Unimodular covariance with \textbf{explicit non-automatic hypotheses} \\
T3 & New packaging & Observation-as-factor formalization \\
T4 & \textbf{New} & \textbf{Conditional complexity bound via causal thick boundary} \\
T5 & Refinement & Brudno alignment~\cite{Brudno1983} with \textbf{normalization discipline} \\
T6--T9 & \textbf{New} & \textbf{Program emergence in static block; halting witness staticization} \\
...
T13 & Refinement & Sofic $\omega$-language with \textbf{explicit Markovizability conditions}~\cite{Buchi,Thomas} \\
T14--T16 & \textbf{New} & \textbf{SBU existence; causal extension; deterministic progression} \\
T17--T18 & \textbf{New} & \textbf{Multi-anchor observers; subjective time rate; coordinate relativization} \\
T19--T20 & \textbf{New} & \textbf{Apparent choice vs determinism unification} \\
T21--T22 & Refinement & Information laws~\cite{LiVitanyi} with \textbf{Følner normalization discipline} \\
...
&\le K(f)+K(W)+K(\pi)+O(\log|W|) \\
&= O(1)+O(\log T)+O(1)+O(\log T)=O(\log T).
\end{align*}
...
\textbf{Compression--entropy experiment (§7.5).} For Rule-110 with random initial condition:
\begin{enumerate}
...
$$
X_f:=\Big\{x\in\Sigma^{\Z^{d+1}}:\ \forall(\mathbf r,t),\ x(\mathbf r,t)=f\big(x(\mathbf r+N,t-1)\big)\Big\}.
$$
...
$$
Y_G=\Big\{(e_t)_{t\in\Z}\in E^{\Z}:\ \forall t,\ \mathrm{tail}(e_{t+1})=\mathrm{head}(e_t)\Big\}.
$$
\end{itemize}
...
\subsection{Eternal-graph coordinate relativityization (Anchored Chart)}
...
\mathrm{Cone}^+_{\ell}(v):=\Big\{w\in \mathrm{Dom}(\varphi_{v_0}):\ \exists v\leadsto w\ \text{and }\ell\ \text{monotone non-decreasing along path}\Big\}.
$$

\textbf{SBU (Static Block Unfolding)}
$$
X_f^{(v,\tau)}:=\Big\{x\in X_f:\ x\big|_{\varphi_{v_0}(\mathrm{Ball}_G(v,R_0))}=\mathrm{pat}(v)\ \text{and}\ x\ \text{is consistent extension in}\ \varphi_{v_0}\!\big(\mathrm{Cone}^+_\ell(v)\big)\Big\},
$$
...
$$
\mathsf{Lang}_{\pi,\varsigma}(X_f):=\Big\{\cO_{\pi,\varsigma}(x)\in\Gamma^{\N}:x\in X_f,\ \text{leaf-by-leaf reading per }\cL\Big\},
\qquad
...
\begin{lemma}[5.5: Time-subaction version SMB/Brudno]\label{lem:5.5}
For \textbf{$\sigma_{\mathrm{time}}$-invariant and ergodic} $\mu$ and \textbf{time-slice cuboid family} $W_k = R \times [t_k, t_k+T_k-1]$, where $T_k\to\infty$, and \textbf{$R\subset\Z^d$ is fixed finite set}.
\end{lemma}
...
Denote $L(W_k)=T_k$ (temporal thickness), then
$$
...
\begin{theoremT}[Unimodular covariance; coordinate-aligned expression of complexity density]\label{thm:T2}
\end{theoremT}

\begin{axiom}[Assumption (U): Unimodular covariance geometric constraints]\label{assump:U}
...
$$
W_k=R\times[t_k,t_k+T_k-1],\quad \tilde W_k=\tilde R\times[\tilde t_k,\tilde t_k+\tilde T_k-1],
$$
...
Below upper bound holds under \S2.2's \textbf{single-step temporal dependence} premise; if rule depends on past $m>1$ layers, correspondingly extend layer-by-layer dependence domain to union of previous $m$ layers, adjusting propagation radius, rest of reasoning unchanged.
\end{note}
...
$$
\bigvee_{i\in\Z}\sigma_{\mathrm{time}}^i\alpha_{R_k}\quad\text{generates entire }\sigma\text{-algebra as }k\to\infty
$$
(i.e., $\{\alpha_{R_k}\}$ forms a \textbf{generating partition sequence}), then
$$
\limsup_{k\to\infty}\frac{K(x|_{W_k})}{T_k}=h_\mu(\sigma_{\mathrm{time}}),\qquad
...
$$
X_f=\mathcal T_f(\mathsf{Conf})=\Big\{x\in\Sigma^{\Z^{d+1}}:\forall(\mathbf r,t),\ x(\mathbf r,t)=f(x(\mathbf r+N,t-1))\Big\}.
$$
\end{theoremT}
...
\textbf{Step 1: Finite-memory reduction.} Under time-direction Markovizability condition, take higher-order block representation $X_f^{[k]}$ such that cross-leaf consistency depends only on adjacent $k$ layers. Let $\Sigma^{[k]}$ denote extended alphabet encoding $k$-blocks.

\textbf{Step 2: Automaton state space.} Define finite state set
$$
Q\ :=\ \big\{[\sigma_0,\ldots,\sigma_{k-1}]:\ (\sigma_0,\ldots,\sigma_{k-1})\text{ forms valid } k\text{-layer configuration}\big\}.
$$
...
$$
\delta(q,a)\ =\ \big\{[\sigma_1,\ldots,\sigma_{k-1},\sigma_k]:\ \pi(\sigma_0,\ldots,\sigma_k)=a\text{ and local SFT constraints satisfied}\big\}.
$$
Here $\pi$ acts on $(k+1)$-layer window (kernel $B$ with thickness $\delta_B\le k$) outputting symbol $a$.
...
\textbf{Conclusion (corrected)}: For fixed finite cross-section $R$ and time-slice cuboid families,
$$
\limsup_{k\to\infty}\frac{K(\pi(x|_{W_k}))}{L(W_k)}
=\frac{1}{b}\,h_{\pi_\ast\mu}\!\big(\sigma_{\mathrm{time}}^{b},\alpha_R^\pi\big)
...
Above analysis holds under \textbf{temporal-thickness $L(W)$} normalization; if instead using voxel-count normalization or non-time-slice-cuboid window families, corresponds to $h_\mu^{(d+1)}$ not time entropy (see \S2.5 and \S7.5 notes).
\end{note}
...
$$
W_k^{+\rho}:=\big\{x\in\Z^d:\exists y\in W_k,\,\|x-y\|_\infty\le\rho\big\}
\quad(\text{or equivalently }W_k+[-\rho,\rho]^d).
$$
...
Let $F$ be radius-$r$ $d$-dimensional CA, take any F\o lner window family $\{W_k\}$ (Definition~\ref{def:Folner}, axis-aligned parallelepipeds satisfying $K(W_k)=O(\log|W_k|)$). Define spatial information density (per cell)
$$
I(x):=\limsup_{k\to\infty}\frac{K(x|_{W_k})}{|W_k|},\qquad
I_\pi(x):=\limsup_{k\to\infty}\frac{K(\pi(x|_{W_k}))}{|W_k|}.
$$
...
=\limsup_{k\to\infty}\frac{K(x|_{W_k^{+rT}})}{|W_k^{+rT}|}\cdot\frac{|W_k^{+rT}|}{|W_k|}=I(x).
$$
...
\textbf{Definition.} To avoid confusion with leaf-family notation $\rho$ in \S2, below use $\eta$ for parameter. Take visible category set (given by decoding/counting rule) indexed $j=1,\ldots,J$ (here $J<\infty$), assign weights $a_j>0$ and vectors $\beta_j\in\R^n$. Define
$$
...
Fix direction $\mathbf v$ and decomposition $\eta=\eta_\perp+s\mathbf v$. Let index family $\{(a_j,\beta_j)\}_{j\ge1}$ be \textbf{countably infinite}, assume exists $\eta_0$ making $\sum_{j\ge1} a_j e^{\langle\beta_j,\Re\eta_0\rangle}<\infty$. Let weighted cumulative distribution along $\mathbf v$ be
$$
...
r_k=\frac{|c_k|}{|W_k|},\quad
\mathrm{plot}(r_k)\quad (k=1,2,\ldots).
$$
...
\textbf{Results.} Fig.~\ref{fig:T5_convergence} shows clear monotonic convergence:
\begin{itemize}
...
Convergence is monotonic and consistent with T5's prediction: computable compression upper-bounds $K(\cdot)$, and normalized rate stabilizes to system's time-subaction entropy. Rule-110's higher rate reflects greater complexity/unpredictability vs Rule-90's periodic/low-entropy dynamics.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{fig1_spacetime_diagrams.pdf}
\caption{\textbf{Spacetime Block Visualizations.} (Left) Rule-110 exhibits complex, irregular patterns characteristic of universal computation. (Right) Rule-90 displays regular, self-similar fractal structure (Sierpiński triangle). Both are static blocks $X_f \subset \{0,1\}^{\mathbb{Z}^2}$ satisfying local rule $f$; ``time evolution'' is leaf-by-leaf reading artifact.}
\label{fig:spacetime}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{fig2_T4_reconstruction.pdf}
\caption{\textbf{T4 Thick Boundary Verification.} (Left) Ground truth window $x|_W$. (Center) Reconstructed from thick boundary alone. (Right) Difference map showing \textbf{zero errors}—perfect reconstruction validates T4's causal dependency analysis and conditional complexity bound.}
\label{fig:T4_reconstruction}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{fig3_T5_entropy_convergence.pdf}
\caption{\textbf{T5 Brudno Convergence.} Normalized compression rate $K(\pi(x|_W))/T$ vs temporal thickness $T$. Both systems exhibit monotonic convergence to finite entropy rates, with Rule-110 (complex) stabilizing at $\approx 0.10$ bytes/step and Rule-90 (linear) at $\approx 0.09$ bytes/step. Log-scale $T$-axis highlights convergence behavior; asymptotic rates reflect time-subaction entropy $h_{\pi_\ast\mu}(\sigma_{\mathrm{time}})$.}
\label{fig:T5_convergence}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{fig4_T17_subjective_time.pdf}
\caption{\textbf{T17 Subjective Time Rate Dependence.} Entropy rate (normalized by temporal thickness $T$, not observation steps) vs step-size $b$. Larger $b$ (``slower'' subjective time) yields \textbf{monotonically lower} information density per unit $T$, consistent with $h_{\mathrm{obs}} \propto 1/b$ prediction. All curves converge as $T \to \infty$, approaching system's intrinsic $h_{\pi_\ast\mu}(\sigma_{\mathrm{time}})$.}
\label{fig:T17_subjective}
\end{figure}

\begin{table}[h]
\centering
\caption{Experimental verification summary for core theorems}
...
\subsection{Interpretation (RPG metaphor) -- \textit{illustrative, non-technical}}

\textbf{Layer separation}: \textbf{operational layer} (observation/decoding/leaf-by-leaf progression/representative selection) versus \textbf{ontological layer} (static geometry/unique consistent extension).

\textbf{Compatibility principle}: regard $X_f$ as \textbf{RPG's complete data and rules}; \textbf{leaf-by-leaf progression} like unlocking plot at \textbf{fixed chapter rhythm $b$}. Player ``choice'' is \textbf{representative selection} among same-layer compatible branches and \textbf{exclusion} of incompatible branches; \textbf{plot ontology} (static block) already written, choice doesn't generate new information (A3), compatible with determinism (T20).

\textbf{Subjective time rate (corrected)}: effective step-size $b=\langle\tau^\star,\tau\rangle$ only changes \textbf{observation step-frequency per unit temporal thickness}, thus normalizing by $L(W)=T$, observation entropy rate is
$$
\frac{1}{b}\,h_{\pi_\ast\mu}\!\big(\sigma_{\mathrm{time}}^{b},\alpha_R^\pi\big),
$$
non-increasing in $b$. When adopting $R_k\uparrow\Z^d$ approaching generating partition, limit is
$$
h_{\pi_\ast\mu}(\sigma_{\mathrm{time}}),
$$
i.e. under generating-partition limit independent of $b$ choice (see T17).
...
No datasets were generated or analyzed during the current study. All mathematical constructions and theoretical results are presented in full within the manuscript and supplementary materials.

\section*{Code availability}
All scripts and computational experiments described in \S7.5 (compression-entropy experiments) are available at \texttt{[repository URL to be provided upon acceptance]}. Reference implementations for SFT construction (\S7.1--7.2), decoder design (\S7.3), and SBU forcing-domain propagation (\S7.6) will be made available under open-source license (MIT or equivalent) upon publication.

\section*{Acknowledgements}
The authors thank [to be filled] for helpful discussions on symbolic dynamics and amenable group actions, and [to be filled] for comments on earlier drafts.

\section*{Author contributions}
Auric: Conceptualization, formal analysis, investigation, methodology, proof development, writing---original draft, writing---review \& editing.
