...
* **叶计数与时间片长方体族**：对时间片长方体族窗口 $W=R\times[t_0, t_0+T-1]$（$R\subset\mathbb{Z}^d$），定义 $L(W)=T$ 为时间厚度（穿越的叶层数）。在步长 $b$ 的协议下，**观察步数**为 $\lfloor L(W)/b\rfloor$（忽略 $O(1)$ 边界效应）。此类窗口族与时间子作用 $\sigma_{\mathrm{time}}$ 的一维Følner理论相容。本文复杂度上界可将该 $O(1)$ 边界效应吸收到 $O(\log|W|)$ 项中（见 T4）。
...
**命题.** （允许经有限高阶块表示/字母扩展）存在宏块-强制嵌入，使任意图灵机 $M$ 的运行可在（扩展后的）$X_f$ 中实现并被 $\pi$ 译读。
...
$$
**并且仅当**上述联结分割生成（或存在沿时间的有限记忆可逆块码同构）**时**，才可写作 $b\,h_{\pi_\ast\mu}(\sigma_{\mathrm{time}},\alpha_R^\pi)$。观察步数计数 $\lfloor L(W)/b\rfloor$ 的 $1/b$ 因子与 $b$ 正好抵消，归一化后熵率不依赖于 $b$ 的选择。由引理 5.5，对 $\mu$-a.e. 的 $x$ 两族密度极限一致。$\square$
...
$$
**注**：此定理针对 $d$ 维空间配置，按体素数归一化。应用于时空SFT的时间子作用时，应改用叶计数归一化（见T5）。
