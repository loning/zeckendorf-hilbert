...
> **注意**：此图展示的是GLS理论**提议的**概念结构，用于说明不同物理量在理论框架内的逻辑联系。
...
|------|------|----------|----------|----------|
| 时间 | $t, \tau$ | 演化的参数 | 依赖于刻度 | [统一时间](../05-unified-time/00-time-overview.md) |
| 相位 | $\varphi$ | 量子旋转角度 | $S/\hbar$ | [数学工具](../03-mathematical-tools/00-tools-overview.md) |
...
| 广义熵 | $S_{\text{gen}}$ | 几何+物质熵 | $A/4G + S_{\text{out}}$ | [IGVP](../04-igvp-framework/00-igvp-overview.md) |
| 因果钻石 | $\mathcal{D}$ | 时空的最小单元 | $J^+(p) \cap J^-(q)$ | [因果钻石](../04-igvp-framework/02-causal-diamond.md) |
| 观察者 | $\mathcal{O}$ | 测量装置/意识 | 自指散射网络 | [矩阵宇宙](../10-matrix-universe/00-intro.md) |
...
- **完全新手** → [基础概念篇](../01-foundation/01-what-is-time.md)
  - 从时间、因果、边界开始，建立直觉

- **有物理背景** → [核心思想篇](../02-core-ideas/01-time-is-geometry.md)
  - 直达五者合一的统一刻度

- **想看数学** → [数学工具篇](../03-mathematical-tools/00-tools-overview.md)
  - 理解散射、谱理论、信息几何

- **想看大图景** → [最终统一篇](../11-final-unification/00-intro.md)
  - 看单一变分原理如何导出所有定律
...
[← 上一篇：阅读指南](01-reading-guide.md) | [返回主页](../index.md) | [开始学习 →](../01-foundation/01-what-is-time.md)
