...
> **GLS理论提出：宇宙可能不是由五个独立的"东西"（时间、因果、边界、散射、熵）组成的，而是同一个深层结构的五种表现形式。就像立方体的五个投影，它们看起来不同，但描述的可能是同一个对象。**
