...
现在我们追问一个更根本的问题:**时间在哪里被定义?**

传统物理认为时间定义在**空间内部**。但GLS理论给出惊人的答案:

> **时间定义在边界上。所有关于时间的信息,都由边界"说"出来。**

就像一本书的内容可以由封面的条形码读出,宇宙的时间结构可以由其边界完全决定。这就是**边界语言**(Boundary Language)的核心思想。
...
- **边界语言**:只要在门框上测量**谁进谁出、带走什么**,就能完全推断出房间内部的状态
...
- **时间不是外加的**,而是由边界上的翻译算子 $\alpha_t$ 生成
- 就像旋转门的"时间" = 门转动的圈数
- **边界即时钟**!
...
**惊人事实**:如果不加GHY边界项,Einstein-Hilbert作用量的变分就**不完备**! 这说明引力从根本上是**边界理论**。
...
在满足边界语言三公理的前提下,**三种时间只是同一边界时间的不同归一化**!
...
**关键**:不需要知道黑洞内部发生了什么,视界边界已经"说"出了全部信息!
...
> **时间不定义在空间内部,而定义在边界上。边界通过"通量、翻译、单调"三公理,完全决定了内部的时间结构。这就是边界语言。**
...
- **Einstein方程需要GHY边界项** → 引力本质上是边界理论
- **黑洞视界完全决定内部** → 信息不在体积中,在表面上
- **时间由边界生成** → "时间流逝"是边界翻译算子的表现
