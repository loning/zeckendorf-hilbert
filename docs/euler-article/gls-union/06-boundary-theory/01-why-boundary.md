...
**物理含义**：
$$\boxed{\text{代数内禀时间（模流）} \cong \text{边界几何生成元}}$$
...
**物理含义**：
$$\boxed{\text{引力作用的可微性} \leftarrow \text{边界项决定}}$$
...
**共同主题**：
$$\boxed{\text{体域} \approx \text{边界的函数（或重建）}}$$
...
> 体域区域 $\mathcal{M}$ 的物理内容理论上可由某个边界三元组 $(\partial\mathcal{M}, \mathcal{A}_\partial, \omega_\partial)$ 完全重建（在给定理论的适用范围内），即时间演化与响应算子均由边界一参数自同构群与态的演化确定。
...
> 体域物理内容理论上可由边界三元组 $(\partial\mathcal{M}, \mathcal{A}_\partial, \omega_\partial)$ 完全重建。
