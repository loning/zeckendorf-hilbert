...
> **物理真正发生的地方是边界。体域内部只是边界数据的"投影"或"全息像"。**
...
- **GLS观点**:舞台**就是边界**,所有"演员"(物理过程)都在边界上表演
...
在满足匹配条件的前提下,存在唯一的统一边界时间生成元 $H_\partial$(至仿射变换),使得:
...
> **物理的真正舞台是边界,不是体域。散射时间延迟、模流演化、Brown-York边界能量是同一边界生成元的三种扮相,统一在边界三元组(∂ℳ, 𝒜_∂, ω_∂)上。**
