...
**定义 1（窗口—局部可逆，WLRC）。** 给定 $F$、窗口 $(i,W)$ 与固定外部邻域模型。若存在**双射** $\Phi_{i,W}:\mathcal S_W\to\mathcal S_W$ 与**选择函数** $s:\mathcal X_W\to\mathcal A$ 使得

$$
\forall u\in\mathcal X_W:\quad
\pi\!\big(\Phi_{i,W}(u,s(u))\big)=y(u),
$$

则称 $F$ 在 $(i,W)$ 上**窗口—局部可逆**，写作"$F$ 的 WLRC 于 $(i,W)$ 存在"。
...
**定理 4（多窗一致性与可逆回溯）。** 设一组重叠窗口 $\{(i_j,W)\}_j$ 覆盖区间 $[L,R]$。若每个窗口采用相同 $P_W$ 与**同步边带协议**（同一空间位置的边带标签在重叠区一致），则在该覆盖区间上存在**一致的逆演化重构**，其必要充分条件是：对所有重叠对 $(i_j,W)\cap(i_{j'},W)\neq\varnothing$，两窗口对重叠坐标的读数与边带满足可满足性约束族；在**相邻链**覆盖情形，成对一致即推得全局一致并唯一。

**证明。** 把每个窗口一步映射视为对其交叠坐标与边带的约束；因各子窗口映射是双射，故**局部节**之间的一致性等价于图上约束的可满足性。线性可加规则（如模 2 线性 CA）时约束为线性方程组，一般规则为布尔关系；链式覆盖中，成对一致即能以"搭接—黏合"方式唯一拼接（集合论层面的预层黏合）。$\square$
...
**复杂度。** 由于 $\mathsf{Bdry}$ 为确定性映射，枚举 $\tilde u$ 的时间复杂度为 $O(2^{W})$。若采用最小字母表 $|\mathcal A|=k_{\max}$，则稀疏存储规模为 $O(2^{W}k_{\max})$；一般情形为 $O(2^{W}|\mathcal A|)$。
