...
\title{Geometry and Operator Networks of Causal Consensus: Nested Causal Diamonds, Boundary Time Geometry, and Matrix Universe}
...
\begin{enumerate}
\item The space--time background is encoded by a nested family of small causal diamonds $(D_{p,r})$ on a globally hyperbolic Lorentzian manifold. Their partial order and generalized entropy arrow define a time-free ``causal manifold'' structure.

\item On the boundary $\partial D$ of each small causal diamond, a boundary algebra, a state and a local scattering matrix $S_D(\omega)$ are assigned. Using the Birman--Kreĭn formula and Wigner--Smith time-delay operator, a unified time scale
$$
\kappa(\omega)
...
\item Observers are modeled as paths $\gamma$ in a causal-diamond complex equipped with a Hilbert bundle and an operator-valued connection $\mathcal{A}$. The experienced world of an observer is given by the path-ordered product
$$
...
\textit{The universe at the ontological level is a time-parameter-free causal manifold; what observers call ``time'' and ``evolution'' arises from the way local operator algebras on small causal diamonds are glued and compared, under finite information capacity.}

\subsection{Scattering theory, spectral shift and unified time scale}
...
$$
where $\varphi(\lambda)$ is the total half-phase of $S(\lambda)$ and $\rho_{\mathrm{rel}}(\lambda)=\xi'(\lambda)$ is the relative spectral density. This quantity $\kappa$ will serve as a \textit{universal time scale} in the present framework.

\subsection{Modular flow, null surfaces and generalized entropy}
...
The universe, from this perspective, is a \textit{matrix universe}: its causal structure and observer experiences are encoded in consistency conditions and curvature properties of $\mathcal{A}$.
...
\subsection{Lorentzian background and small causal diamonds}

Let $(M,g)$ be a four-dimensional, oriented, time-oriented Lorentzian manifold with signature $(-+++)$, satisfying:

\begin{enumerate}
\item \textbf{Global hyperbolicity.} There exists a Cauchy surface $\Sigma\subset M$ such that every inextendible causal curve intersects $\Sigma$ exactly once.

\item \textbf{Stable causality.} There exists a smooth time function $T\colon M\to\mathbb{R}$ strictly increasing along future-directed timelike curves; no closed causal curves exist.

\item \textbf{Curvature scale.} For each point $p\in M$, a local curvature scale $L_{\mathrm{curv}}(p)$ is defined such that in normal coordinates $g_{ab}(x)=\eta_{ab}+O(|x|^2/L_{\mathrm{curv}}^2)$ as $|x|\to 0$.
\end{enumerate}
...
\begin{axiom}[G: Geometric Axiom]
\begin{enumerate}
\item $(M,g)$ satisfies global hyperbolicity and stable causality as above.

\item For all $p$ and sufficiently small $r$, the diamond $D_{p,r}$ is causally convex and diffeomorphic to a diamond in Minkowski space; deviations of volumes and areas from the flat case are $O(r^2)$.
\end{enumerate}
The family of all such diamonds $\mathcal{D}=\{D_\alpha\}_{\alpha\in A}$ will be called a \textit{small-diamond family}.
\end{axiom}

\subsection{Causal-diamond complex and causal gaps}

Given a small-diamond family $\mathcal{D}$, define its Čech nerve $\mathsf{K}(\mathcal{D})$ as follows:

\begin{itemize}
\item Vertices correspond to nonempty diamonds $D_\alpha$.

\item A $k$-simplex $[\alpha_0\cdots\alpha_k]$ is present whenever $D_{\alpha_0}\cap\cdots\cap D_{\alpha_k}\neq\varnothing$.
\end{itemize}
...
\begin{assumption}[C: Čech Causal Consistency]
...
Deviations from zero can be interpreted as \textit{causal gaps}; they will be quantified in terms of an entropy density along null generators in Section 3.

\subsection{Boundary-time geometry and unified time scale}

On each diamond boundary $\partial D$, we consider a Hilbert space $\mathcal{H}_{\partial D}$ describing the degrees of freedom crossing the null boundary (for instance, the one-particle Hilbert space of a free field restricted to the boundary, or an appropriate Fock-space factor). We then assign:

\begin{itemize}
\item A von Neumann algebra $\mathcal{A}_{\partial D}\subset\mathcal{B}(\mathcal{H}_{\partial D})$ of boundary observables;

\item A reference state $\omega_{\partial D}$ (e.g. the vacuum or a thermal state);

\item A scattering matrix $S_D(\omega)\in\mathcal{U}(\mathcal{H}_{\partial D})$ depending measurably on a spectral parameter $\omega\in X^\circ$ (such as energy, momentum or other quantum numbers).
\end{itemize}
...
\begin{axiom}[T: Time-scale Axiom]
...
=\frac{1}{2\pi}\operatorname{tr}Q_D(\omega),
$$
and the corresponding modular flow parameter for the state $\omega_{\partial D}$ restricted to $\mathcal{A}_{\partial D}$ is affinely equivalent to $\omega\mapsto\int^\omega\kappa_D(\omega')\,\mathrm{d}\omega'$. In particular, the ``scattering time'', ``modular time'' and ``geometric time'' based on Brown--York boundary Hamiltonians lie in the same equivalence class $[\tau]$.
\end{axiom}

This axiom formalizes the unification of time scales from scattering, modular theory and boundary geometry.

\subsection{Observers as local causal fragments}

Let $X$ denote the abstract set of events in the causal manifold, endowed with the global partial order $\prec$. An \textit{observer} $O_i$ is modeled as a multi-component structure
$$
...
\begin{itemize}
\item $C_i\subset X$ is the observer's accessible causal domain;

\item $\prec_i$ is a local partial order on $C_i$, compatible with $\prec$;

\item $\Lambda_i$ is a resolution scale (cutoff function on spacetime and frequency);

\item $\mathcal{A}_i\subset\mathcal{B}(\mathcal{H}_i)$ is the observer's accessible algebra;

\item $\omega_i$ is the observer's state (belief state) on $\mathcal{A}_i$;

\item $\mathcal{M}_i$ is a model family (parametrized dynamics or hypotheses);

\item $U_i$ is an update or learning operator (possibly CPTP);

\item $u_i$ is a preference or utility functional;

\item $\mathcal{C}_{ij}$ encodes communication channels between observers $i$ and $j$.
\end{itemize}
...
\subsection{Null--Modular double cover and $\mathbb{Z}_2$ sectors}
...
At the same time, feedback loops and self-referential scattering networks can produce a square-root branch structure for scattering determinants, defining a principal $\mathbb{Z}_2$ bundle with holonomy
$$
...
\begin{axiom}[A: Topological Axiom]
In the region and energy window of interest, the $\mathbb{Z}_2$ obstruction class vanishes:
$$
[K]=0,\quad
...
This ensures that the matrix universe is free of discrete $(\mathbb{Z}_2)$ anomalies and that path holonomy depends continuously on the connection curvature alone.

\section{Main Results: Geometry of Matrix Universe and Causal Consensus}

\subsection{Reconstruction from small causal diamonds}
...
\begin{theorem}[Causal-diamond reconstruction]
...
\begin{enumerate}
\item The geometric realization $|\mathsf{K}(\mathcal{D})|$ of the Čech nerve is homotopy equivalent to $M$.

\item The global causal partial order $\prec$ on $M$ is uniquely determined by the family of local partial orders $\{\prec_\alpha\}$ via transitive closure of the relation
$$
xRy\iff\exists\alpha:\ x,y\in D_\alpha,\ x\prec_\alpha y.
$$

\item In the limit of arbitrarily small diamonds, the metric $g$ can be reconstructed (up to diffeomorphism) from the volumes, edge areas and entanglement properties of the diamonds.
\end{enumerate}
\end{theorem}
...
\subsection{Matrix universe: Hilbert bundle and operator connection}
...
\begin{theorem}[Existence and uniqueness of matrix universe]
Under Axioms G, T and A, and the above consistency conditions, there exist:

\begin{enumerate}
\item A Hilbert bundle $\pi\colon\mathcal{H}\to Y$ with local trivializations agreeing with $\mathcal{H}_\alpha$ on $U_\alpha\times X^\circ$;

\item A global unitary-valued field $S(\omega;x,\chi)\in\mathcal{U}(\mathcal{H}_{(x,\chi)})$;

\item An operator-valued connection one-form
$$
...
$$
will be called a \textit{matrix universe}.

\subsection{Causal consensus for observer paths}
...
\begin{theorem}[Strong causal consensus in almost flat matrix universes]
...
\begin{enumerate}
\item $\gamma_1,\gamma_2\subset\Omega$, with $\gamma_1(0)=\gamma_2(0)$, $\gamma_1(1)=\gamma_2(1)$, and $\gamma_1$ homotopic to $\gamma_2$ within $\Omega$;

\item $\lVert\mathcal{F}\rVert_{L^\infty(\Omega\times I)}\le\delta$;

\item The $\mathbb{Z}_2$ obstruction class $[K]=0$ on the relevant portion of $M\times X^\circ$.
\end{enumerate}
...
The proof uses a non-Abelian Stokes theorem and curvature estimates, together with the triviality of $\mathbb{Z}_2$ holonomy ensured by $[K]=0$. Details are deferred to Appendix C.

\subsection{Causal gaps and Markov defect}
...
$$
I(D_{j-1}:D_{j+1}\mid D_j)
=\iint\iota(v,x_\perp)\,\mathrm{d}v\,\mathrm{d}^{d-2}x_\perp.
$$

\begin{definition}[Causal gap density]
...
G[D_{j-1},D_j,D_{j+1}]
:=I(D_{j-1}:D_{j+1}\mid D_j)
=\iint\mathfrak{g}(v,x_\perp)\,\mathrm{d}v\,\mathrm{d}^{d-2}x_\perp
$$
...
\subsection{Information-capacity bound}
...
Within the matrix-universe description, the degrees of freedom accessible inside $\mathcal{R}$ form an effective Hilbert space $\mathcal{H}_{\mathcal{R}}$ obtained by restricting $\mathcal{H}$ to $Y_{\mathcal{R}}=\mathcal{R}\times X^\circ$ and imposing UV and IR cutoffs appropriate to the unified time scale.

\begin{theorem}[Matrix-universe information-capacity bound]
Under Axioms G, T, A, and assuming semi-classical gravity with generalized entropy $S_{\mathrm{gen}}[\partial\mathcal{R}]$ finite, the dimension of the effective Hilbert space $\mathcal{H}_{\mathcal{R}}$ satisfies
$$
\log\dim\mathcal{H}_{\mathcal{R}}
...
\item \textbf{Metric reconstruction.} Standard results show that the causal order and local volume information fix $g$ up to conformal factor; the conformal factor can be recovered from volume densities, or, more refined, from entanglement entropies in small balls using Jacobson's entanglement-equilibrium argument.
\end{enumerate}

Details, including the handling of nontrivial causal boundaries and curvature corrections, are given in Appendix A.

\subsection{Proof strategy for Theorem 3.2}

For Theorem 3.2, the argument proceeds as follows:

\begin{enumerate}
\item \textbf{Čech data for Hilbert bundles.} The local Hilbert spaces $\mathcal{H}_\alpha$ and unitaries $U_{\alpha\beta}$ define a Čech 1-cocycle with values in the unitary group of infinite-dimensional Hilbert space. Standard bundle theory produces a Hilbert bundle $\mathcal{H}\to Y$ whose restriction to $U_\alpha\times X^\circ$ is isomorphic to $\mathcal{H}_\alpha$.

\item \textbf{Global scattering field.} On overlaps, the local scattering matrices transform as
$$
S_\alpha=U_{\alpha\beta}S_\beta U_{\alpha\beta}^\dagger,
$$
ensuring that the local definitions patch to a global $S(\omega;x,\chi)$.

\item \textbf{Connection construction.} Define on each local trivialization
$$
\mathcal{A}_\alpha
...
$$
\mathcal{A}_\alpha\mapsto V_\alpha\mathcal{A}_\alpha V_\alpha^\dagger + V_\alpha\,\mathrm{d} V_\alpha^\dagger,
$$
which is the usual transformation law for a connection. The cocycle relations for $U_{\alpha\beta}$ ensure that the $\mathcal{A}_\alpha$ glue to a global connection $\mathcal{A}$.

\item \textbf{Unified time scale.} The frequency component of $\mathcal{A}$ reproduces $Q=-iS^\dagger\partial_\omega S$, whose trace yields $\kappa(\omega)$. Under Axiom T, this matches the modular and geometric times, giving a unified time scale.
\end{enumerate}
...
\subsection{Proof strategy for Theorem 3.3}
...
\begin{enumerate}
\item \textbf{Non-Abelian Stokes theorem.} For a connection $\mathcal{A}$ with curvature $\mathcal{F}$, the holonomy around a small loop is approximated by
$$
...
\item \textbf{Curvature bound.} If $\lVert\mathcal{F}\rVert\le\delta$ and $\Sigma$ has area $\mathrm{Area}(\Gamma)$, then a Grönwall-type estimate yields
$$
\bigl|\mathcal{U}(\Gamma)-\mathbb{I}\bigr|
\le C\,\delta\,\mathrm{Area}(\Gamma).
$$

\item \textbf{Gauge-invariant distance.} The gauge-invariant distance between $U_{\gamma_1}$ and $U_{\gamma_2}$ is controlled by $|\mathcal{U}(\Gamma)-\mathbb{I}|$. Triviality of the $\mathbb{Z}_2$ holonomy removes possible sign flips, leaving only the continuous part controlled by curvature.
\end{enumerate}

The detailed inequalities and gauge-invariance properties are established in Appendix C.

\subsection{Proof strategy for Theorem 3.5}

For Theorem 3.5, one combines:

\begin{enumerate}
\item The holographic-type bounds relating generalized entropy $S_{\mathrm{gen}}$ to the logarithm of Hilbert-space dimension in semi-classical gravity.

\item The identification of the effective Hilbert factor $\mathcal{H}_{\mathcal{R}}$ in the matrix-universe description with the subspace of $\mathcal{H}$ localized in $\mathcal{R}$ and in the relevant frequency window, once appropriate cutoffs are chosen.
\end{enumerate}

This yields $\log\dim\mathcal{H}_{\mathcal{R}}\lesssim S_{\mathrm{gen}}$, with constants encoding the choice of cutoffs and the unified time scale. A more refined statement would relate the local density of matrix degrees of freedom to the integrand of the generalized entropy functional.

\section{Model Applications}

\subsection{Continuum quantum field theory on curved space--time}
...
The boundary Hilbert space $\mathcal{H}_{\partial D}$ consists of modes localized on the null boundary, and the scattering matrix $S_D(\omega)$ expresses how these modes are mixed by the dynamics inside $D$. The Wigner--Smith operator $Q_D(\omega)$ encodes the group-delay (time-delay) distribution associated with these modes, and its trace yields the local contribution to the unified time scale $\kappa(\omega)$.
...
\subsection{Matrix universe and large-$N$ models}
...
\section{Engineering Proposals: Causal-Consensus Computing Networks}
...
\subsection{Analog scattering networks}
...
\subsection{Digital matrix-universe simulators}
...
\subsection{Error budgets and consensus thresholds}
...
\section{Discussion: Relations, Limitations and Outlook}

\subsection{Relation to causal set and boundary constructions}
...
\subsection{Relation to modular theory and QNEC/QFC}
...
\begin{itemize}
\item Identifying the modular parameter with the unified time scale $\kappa$.

\item Interpreting QNEC bounds as inequalities constraining the causal gap density $\mathfrak{g}$.

\item Viewing modular Markov properties as the vanishing of curvature-like invariants along null chains.
\end{itemize}
...
\begin{enumerate}
\item \textbf{Dynamical law for the connection.} The present framework treats $\mathcal{A}$ kinematically, as a connection derived from scattering and modular data. A full dynamical theory would specify field equations for $\mathcal{A}$ and its curvature, possibly linked to Einstein's equations and matter dynamics.

\item \textbf{Microscopic definition of small diamonds.} The small-diamond family is assumed but not derived from a more fundamental discrete structure. In causal set or lattice models, one must define and control the continuum limit carefully.

\item \textbf{Backreaction and nonunitarity.} In realistic scenarios with dissipation, decoherence and gravitational backreaction, local scattering maps may not be strictly unitary. Extending the connection formalism to completely positive maps and to non-Hermitian generators is necessary.

\item \textbf{Topological sectors.} The triviality of $[K]$ was assumed. Incorporating nontrivial $\mathbb{Z}_2$ sectors and their physical meaning (e.g. fermionic statistics, spin structures or self-referential scattering loops) remains an important extension.

\item \textbf{Universality and uniqueness.} Whether all physically reasonable space--times and quantum field theories admit a matrix-universe description with unified time scale and causal consensus properties, or whether this selects a special subclass, is an open question.
\end{enumerate}
...
\begin{itemize}
\item The \textit{causal manifold} is a nested network of small diamonds with partial orders and entropic arrows.

\item The \textit{matrix universe} is a Hilbert bundle with operator-valued connection whose parallel transport encodes how observers stitch local scattering and modular information into a global view.

\item \textit{Causal consensus} arises when the connection is almost flat and topologically trivial in the relevant region and energy window, so that different observer paths between the same endpoints yield equivalent unitaries.
\end{itemize}

The slogan ``causal consensus = a huge matrix computation'' thus acquires a geometric and operator-theoretic meaning: global causal consistency is equivalent to the flatness (up to controlled curvature and holonomy) of an underlying matrix-valued connection defined by local scattering and modular data.
...
\begin{itemize}
\item Small causal diamonds and their local partial orders reconstruct the causal manifold.

\item Boundary algebras, states and scattering matrices on diamonds define a Hilbert bundle with an operator-valued connection, whose frequency component yields a unified time scale $\kappa$.

\item Observers correspond to paths in this bundle; their experienced worlds are encoded by path-ordered unitaries $U_\gamma$.

\item Causal consensus between observers is characterized by controlled equivalence of these unitaries for homotopic paths, governed by curvature and $\mathbb{Z}_2$ holonomy constraints.

\item Generalized entropy bounds limit the matrix size associated with finite causal regions, tying information capacity to area and curvature.
\end{itemize}
