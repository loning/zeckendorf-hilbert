...
并且 $\mu=\Lambda/(8\pi G T)$ 由"端点固定+腰面固定+$\delta T/T=\mathcal O(\varepsilon^2)$"唯一确定。

**证明草图**（3行）：变分分解为 $\delta S_{\rm gen}=(\delta S_{\rm gen}|_{\ker\delta V})+(\partial S_{\rm gen}/\partial V)\,\delta V$。在 $\ker\delta V$ 子空间，拉格朗日乘子 $\mu$ 仅消除重标定冗余（$T\to\alpha T$、$V\to V$），不对一阶欧拉方程 $\delta(A/(4G\hbar)+S_{\rm out})=0$ 贡献新物理信息。投影到 $\ker\delta V$ 的正交分解表明对偶项 $-\Lambda V/(8\pi G T)$ 在一阶层只负责钉住温标，未参与 $R_{kk}=8\pi G T_{kk}$ 的导出。□
...
![图 1：显式可交换极限的数值验证。**示例族满足 $C_{\sigma,0}=\mathcal O(\varepsilon)$**（强对称族）。归一化误差上界 $|\delta A+\int\lambda R_{kk}|/\ell^{d-2}$ 随尺度分离参数 $\varepsilon$ 的变化，显示 $\varepsilon^3$ 标度。三条曲线对应不同曲率参数组合：低曲率 $(C_R=0.1, C_{\mathcal C}=0.1)$、中曲率 $(C_R=0.2, C_{\mathcal C}=0.2)$、高曲率 $(C_R=0.3, C_{\mathcal C}=0.3)$，均取 $\lambda_0=1$、$C_{\sigma,0}=0.05\varepsilon$。灰色虚线为参考 $\varepsilon^3$ 标度线。**一般族 $C_{\sigma,0}=\mathcal O(1)$ 的误差标度见盒装上界**，主链推导同样成立但数值图需调整参数（将显示 $\varepsilon^2$ 或常数级标度）。复现参数与代码见`scripts/generate_igvp_figure1.py`（调整 $C_R, C_{\mathcal C}, C_{\sigma,0}$ 三参数即可生成不同族的曲线；数据表见附录J）。](igvp_figure1_exchangeable_limit.png)
...
**命题 2B（小钻石模哈密顿近似的一致控制——主链技术脊梁）**：设背景满足§0的正则与尺度分离假设。令 $\mathfrak G$ 为满足端点固定与腰面固定的几何形变族，令 $\mathfrak S$ 为满足Hadamard与局域KMS的态族。则存在常数 $C_{\rm th}=C_{\rm th}(C_R,C_{\nabla R};d,c_\lambda)$ 与 $\ell_0>0$，对所有 $\ell<\ell_0$ 与 $(\delta g,\delta\text{state})\in\mathfrak G\times\mathfrak S$：
...
**证明草图（2-3行）**：使用Brown-York荷在腰面上的积分表达式 $M=\int_{\Sigma_\ell}(K-K_0)\,dA$，其中 $K$ 为外曲率。固定端点位置使边界项 $\int_{\partial\Sigma_\ell}$ 消失；固定腰面时刻使Boost生成元在 $\Sigma_\ell$ 上的积分为零，故 $\delta J=\delta P=0$。对角动量 $J$ 与线性动量 $P$ 同理。由此小钻石的端点+腰面固定自然落在code subspace假设内。□
