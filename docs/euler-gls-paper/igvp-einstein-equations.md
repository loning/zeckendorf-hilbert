...
建立以相对熵与 Fisher 信息为核心的变分原理。对流形每一点的小尺度因果钻石 $B_\ell(p)$（$\ell\ll L_{\rm curv}$）在定体积约束下引入广义熵泛函 $S_{\rm gen}=S_{\rm grav}+S_{\rm out}+S_{\rm ct}$，并规定一阶驻值与二阶正定性为基本准则：一阶信息平衡 $\delta S_{\rm gen}=0$，二阶相对熵非负 $\delta^2 S_{\rm rel}\ge0$。在明确的域前提（尺度分离、Hadamard 正则、重整化与变分良定）之内，证明一阶条件与方程 $G_{ab}+\Lambda g_{ab}=8\pi G\,T_{ab}$ 等价；在 JLMS 且 $\mathcal F_Q=\mathcal E_{\rm can}$ 的前提下，二阶条件 $\delta^2 S_{\rm rel}\ge0$ 推出 $\mathcal E_{\rm can}[h,h]\ge0$（Hollands–Wald 线性稳定性），而线性化爱因斯坦方程来自一阶层的球/钻石族约束与张量化闭包。等价关系通过两条物理独立的路径冗余闭合：局域热力学—几何光学链与纠缠第一定律—相对熵链。进一步给出 Fisher–Rao 度量的推前构造作为互补证据，并说明在高阶引力（Wald/Dong 熵）及因果钻石第一定律下的自然扩展与一致性。
...
$$
S_{\rm gen}=\frac{A(\partial B_\ell)}{4G\hbar}+S_{\rm out}^{\rm ren}[\rho_{B_\ell};g]+S_{\rm ct}[g],
$$

其中 $S_{\rm out}^{\rm ren}$ 通过点分裂重整化有限，面积型散度与局域反常吸收到 $G_{\rm ren}$ 与 $S_{\rm ct}$。**下文一律记 $G\equiv G_{\rm ren}$，并以此写作 $S_{\rm grav}=A/(4G\hbar)$。**
...
$$
\delta^2 S_{\rm rel}=\mathcal F_Q(\delta\rho,\delta\rho)\ge0.
$$

**（A）有对偶层** JLMS 等价给出边界相对熵等于体相对熵；进一步有 $\mathcal F_Q=\mathcal E_{\rm can}[h,h]$。因此，在背景满足 $G_{ab}+\Lambda g_{ab}=8\pi G\,T_{ab}$ 时，在满足线性化约束与恰当边界条件的扰动 $h_{ab}$ 上，

$$\mathcal F_Q(\delta\rho,\delta\rho)=\mathcal E_{\rm can}[h,h],\qquad\boxed{\ \delta^2 S_{\rm rel}=\mathcal E_{\rm can}[h,h]\ \ge 0\ },$$

因而二阶正定性提供线性稳定性判据（在 $\delta M=\delta J=\delta P=0$ 的子空间）。线性化爱因斯坦方程 $\delta G_{ab}[h]=8\pi G\,\delta T_{ab}$ 则来自一阶层的球/钻石族约束与张量化闭包，不由 $\mathcal E_{\rm can}[h,h]\ge0$ 单独推出。

**（B）无对偶保底层** 沿任意零生成元的局域二阶形状导数满足

$$\boxed{\ \langle T_{kk}(p)\rangle \ge \frac{\hbar}{2\pi}\lim_{A_\perp\to0}\frac{\partial_\lambda^2 S_{\rm out}}{A_\perp}\ }\quad\text{（按单位横截面积归一）},$$

为二阶正性提供 QFT 内部下界；配合相对熵单调性与形变模态哈密顿量得到的 ANEC，实现非全息的独立支撑。

**定理 4.1（IGVP 二阶正定—稳定性）** 在定理 3.1 的驻点背景上，若满足 JLMS 且 $\mathcal F_Q=\mathcal E_{\rm can}[h,h]$，则对所有满足线性化约束与边界条件的扰动 $h$ 有

$$\boxed{\ \delta^2 S_{\rm rel}=\mathcal E_{\rm can}[h,h]\ \ge 0\ },$$
...
**(i) 方案独立** 采用 Hadamard 正则与点分裂重整化，使 $S_{\rm out}^{\rm ren}$ 有限，散度由 $G$ 与 $S_{\rm ct}$ 吸收，从而 $S_{\rm gen}$ 在给定阶对方案不敏感。
...
**D.3 Hollands–Wald 判据** 在 $\delta M=\delta J=\delta P=0$ 子空间，$\displaystyle \mathcal E_{\rm can}=\delta^2 M-\sum_A\Omega_A\,\delta^2 J_A-\frac{\kappa}{8\pi G}\,\delta^2 A$，$\ \mathcal E_{\rm can}\ge0$ 当且仅当线性稳定。

---

## 附录 E QNEC/ANEC 的无对偶支撑

**E.1 QNEC** 沿零生成元的二阶形状导数满足
