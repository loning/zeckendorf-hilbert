...
% Title information
\title{Self-Referential Scattering and the Birth of Fermions:\\
Riccati Square Roots, Spinor Double Cover, and a $\mathbb{Z}_2$ Exchange Phase}
\author{Haobo Ma$^1$ \and Wenlin Zhang$^2$\\
\small $^1$Independent Researcher\\
\small $^2$National University of Singapore}

\date{Version 2.29 --- \today}
...
$$
a natural $\mathbb{Z}_2$ invariant, where for single channel $\det S=S=e^{2i\delta}$; for multichannel/partial waves interpret as $\det/\det_p S$ as needed. $\deg(\det S|_\gamma)=\frac{1}{2\pi i}\oint_\gamma (\det S)^{-1}d(\det S)$. Square-root existence characterized by map-level covering lift condition $[S]\in 2H^1(X^\circ;\mathbb{Z})$ (since $U(1)\simeq K(\mathbb{Z},1)$ and square cover corresponds to multiplication by two on $H^1$); $\nu_{\sqrt{S}}$ given by holonomy of principal $\mathbb{Z}_2$-bundle $P_{\sqrt{S}}=S^*(p)$. Line-bundle-level square-root problem ($c_1(\mathcal{L})\in 2H^2(X^\circ;\mathbb{Z})$) characterized by Bockstein of exponential sequence, targets different objects and generally not mutually deducible from map lifting. Spectral-theoretically, under short-range and no-zero-energy-resonance conditions, combining Birman--Kre\u{\i}n formula with spectral flow yields mod-2 Levinson relation
$$
\nu_{\sqrt{S}}(\gamma)=\exp\Bigl(-i\pi\oint_\gamma d\xi\Bigr)=(-1)^{\mathrm{Sf}(\gamma)}=(-1)^{N_b(\gamma)}.
$$
Functional-analytically, under boundary triple and Nevanlinna--M\"obius structure, rigorize self-referential closed loop
$$
L=\Phi_{\tau,E}(L)=\mathcal{B}_\tau\big(M(E;L)\big),\qquad \mathcal{B}_\tau\in \mathrm{PSL}(2,\mathbb{R}),
$$
giving existence theorem and two-fixed-point exchange in hyperbolic region, proving exchange parity consistent with $\nu_{\sqrt{S}}$. Using one-dimensional $\delta$-potential and Aharonov--Bohm model as examples, give explicit winding number calculations, unifying complex small loops and real fold-back paths via ``discriminant mod-2 intersection.'' For topological superconductor endpoint scattering, distinguish Altland--Zirnbauer symmetry classes: Class D's $\operatorname{sgn}\det r(0)$ and Class DIII's $\operatorname{sgn}\operatorname{Pf}r(0)$ each equivalent to branch sign of $\sqrt{\det r(0)}$. This framework directly applicable to Fermi/Bose statistics in $d\ge 3$; in $d=2$ gives $\mathbb{Z}_2$ projection of anyon $U(1)$ statistics.

\textbf{Experimentally readable mod-2 indicator: Morphological criterion (single-shot $\mathbb{Z}_2$ readout protocol):} 

\textit{Declaration:} The following is an operational readout protocol, not part of the theoretical proof. This protocol maps the $\nu_{\sqrt{S}}(\gamma)\in\{\pm1\}$ of this work to directly observable experimental signs.

In gate-tunable Josephson junctions, when Andreev channel number $\lesssim 4$, perform single $2\pi$ scan of superconducting phase difference $\phi$ at zero energy bias; each time crossing Majorana crossing event, the \textbf{dimensionless $\mathbb{Z}_2$ index}
$$
G_{\mathbb{Z}_2}\equiv \nu_{\sqrt{S}}\in\{+1,-1\}
$$
flips once; this index obtainable from \textbf{normalized readout} of zero-bias conductance or interference signal, realizing \textbf{single-shot measurement} type $\mathbb{Z}_2$ magnetometer for crossing events.
\end{abstract}

\noindent\textbf{Keywords:} Scattering phase square root; $\mathbb{Z}_2$ holonomy; Covering lift; First Chern class parity; Bockstein; Spectral shift; Birman--Kre\u{\i}n; Riccati; Boundary triple; Pfaffian index; Aharonov--Bohm scattering

\noindent\textbf{MSC 2020:} Primary 81U20, 81Q10, 55R10; Secondary 47A40, 81R25, 46L87

\section{Notation, Assumptions, Objects and Core Physical Picture}

\subsection{Core Idea and Physical Picture}

Core idea of this work: unify three seemingly different negative-sign sources using unified $\mathbb{Z}_2$ holonomy index
$$
...
\textbf{General notation hint:} This work uses symbol $L$ in two different contexts. In \S3, $L=\psi'/\psi$ is the Riccati variable. In \S7, $L$ is a boundary parameter taking values on the extended real line. The two usages are separate within their respective sections and do not mix. When mentioning ``self-referential closed loop $L=\Phi_{\tau,E}(L)$,'' it always refers to the boundary parameter from \S7.
...
\textit{Proofs:} By Alexander duality and Poincaré--Lefschetz duality. See Appendix D.

\subsection{Connection and Winding Number}

\textbf{Unique convention (Throughout):} In this work, the notation ``$\sqrt{S}$'' \textbf{always} refers to the \textbf{map-level square-root covering} of $\mathfrak{s}:X^\circ\to U(1)$ (i.e., the monodromy of principal $\mathbb{Z}_2$-bundle $P_{\sqrt{\mathfrak{s}}}=\mathfrak{s}^*(p)$), \textbf{not} a matrix square root. Phrases like ``branch sign of $\sqrt{\det r(0)}$'' always mean the \textbf{sign of the covering monodromy branch}, not taking matrix square roots of $\det r(0)$. See \S2 (covering--lift criterion) and \S9 (D/DIII indices).
...
$$
where $\oint d\xi_p$ is interpreted in Lebesgue--Stieltjes sense. The two definitions are equivalent.
\end{definition}
...
$$
see \S4; while main Theorem~\ref{thm:main} connecting $N_b,I_2$ holds only at $\mathbb{Z}_2$ level:
$$
...
Potential $V$ belongs to short-range class: in $d=1$ (and with certain additional conditions for some $d=2$ cases) ensures $S(E,\lambda)-\mathbf{1}$ is trace-class; for more general $d\ge 2$ cases typically only obtain $S(E,\lambda)-\mathbf{1}$ belongs to suitable Schatten class, thus need to use modified Fredholm determinant $\det_p$ and its continuous branch to define spectral shift. Below for brevity denote uniformly as ``$\det/\det_p$''. Other assumptions remain unchanged: $(E,\lambda)\mapsto S$ piecewise $C^1$ along closed path $\gamma$, and $\gamma$ avoids thresholds and embedded eigenvalues; if cannot completely avoid thresholds, describe via mod-2 intersection number. Single channel $S=e^{2i\delta}$; multichannel/partial waves use $\det/\det_p S$ as overall phase exponential.
...
\subsection{Birman--Kre\u{\i}n and Spectral Shift}
...
$$
When $\gamma$ simultaneously changes energy and external parameter, $\oint_\gamma d\xi$ taken from continuous branch of (modified) Fredholm determinant; reverse orientation gives $\oint_\gamma d\xi\mapsto -\oint_\gamma d\xi$, not changing parity.

\subsection{Dimension--Decay--Determinant and Regularization (Minimal Comparison Table)}
...
$$
\mathrm{Sf}(\gamma)=\deg(\det S|_\gamma)=-\oint_{\gamma_\varepsilon} d\xi\in\mathbb{Z}.
$$
...
$$
$\mathrm{Sf}(\gamma)$ is spectral flow of eigenphase with respect to reference phase. Thus

\textbf{Proof (mod 2):} By Birman--Kre\u{\i}n formula, on absolutely continuous spectral energy segment exists continuous spectral shift $\xi$ such that $\det S=e^{-2\pi i\,\xi}$. Taking continuous branch along closed path $\gamma$, have
$$
...
$$
Let $D\subset X$ be discriminant composed of events like ``upper-half-plane Jost zero generation/annihilation, threshold anomaly, embedded eigenvalue, channel opening/closing''. By \S5's definition, $I_2(\gamma,D)=\langle w_D,[\gamma]\rangle$ is defined for any closed path $\gamma\subset X^\circ$. If exists piecewise $C^1$ 2-chain $\Sigma$ transverse to $D$ with $\partial\Sigma=\gamma$, then each intersection point corresponds to exactly one eigenphase first-order crossing at reference phase, spectral flow jumps by $\pm1$, and $I_2(\gamma,D)=\#(\Sigma\cap D)\bmod2$; thus
$$
(-1)^{\mathrm{Sf}(\gamma)}=(-1)^{I_2(\gamma,D)}.
$$
By definition $N_b(\gamma):=I_2(\gamma,D)$, combining above
$$
(-1)^{\deg(\det S|_\gamma)}=(-1)^{\mathrm{Sf}(\gamma)}=(-1)^{N_b(\gamma)}=(-1)^{I_2(\gamma,D)}.\quad\Box
$$
\textit{(Integer level equality $\mathrm{Sf}=\deg(\det S|_\gamma)$ see \S4; definition and properties of $I_2$ see \S5.)}

\textbf{Note:} For single channel $\det S=S$, above reduces to $\exp\Bigl(i\oint_\gamma \tfrac{1}{2i}S^{-1}dS\Bigr)$.

\textbf{Robustness:} Parity $\nu_{\sqrt{S}}$ insensitive to $C^0$-small perturbation of $\gamma$ relative to $D$ (\textit{cf.} Appendix D). Under maintaining short-range and (modified) determinant continuous branch assumptions and applying small-norm potential perturbation, continuous choice of spectral shift function $\xi$ can be maintained, thus value of $\exp\Bigl(-i\pi\oint_\gamma d\xi\Bigr)$ unchanged, i.e., \textbf{mod-2} spectral flow remains unchanged.

\textbf{Note (integer level):} This work at integer level only asserts $\mathrm{Sf}(\gamma)=\deg(\det S|_\gamma)$ (see \S4). Relation with $N_b(\gamma)$, $I_2(\gamma,D)$ only takes their parity, thus main theorem equalities should be understood as $\mathbb{Z}_2$ level equivalence.
\end{theorem}
...
\label{def:holonomy}
The ``holonomy'' of principal $\mathbb{Z}_2$-bundle $P_{\sqrt{\mathfrak{s}}}=\mathfrak{s}^*(p)$ refers to single-valued-ness of the cover's lift along closed paths. Namely, take $\tilde{\gamma}$ to be the lift of $\gamma_\varepsilon$ in $P_{\sqrt{\mathfrak{s}}}$; if starting sheet labeled $+1$, then ending sheet labeled $\pm 1$. Denote this sign as $\operatorname{Hol}_{P_{\sqrt{\mathfrak{s}}}}(\gamma_\varepsilon)\in\{\pm1\}$. Then
$$
...
\section{Riccati Variable, Weyl--Titchmarsh and Jost Structure}

Let $L=\psi'/\psi$, then
$$
L'+L^2=V-E.
$$
Weyl--Titchmarsh $m$-function and abstract Weyl function $M(z)$ belong to Herglotz or Nevanlinna class. In one-dimensional solvable models, choose Jost function $f$ such that
$$
...
\section{Birman--Kre\u{\i}n, Spectral Shift and Mod-2 Levinson}
...
$$
\textit{Hint:} Follows from analyticity of modified Fredholm determinant and continuous choice of spectral shift function (see [9],[10]).

Above notation consistent with ``$\det/\det_p$'' in \S0.4.
\end{theorem}

\begin{theorem}[Birman--Kre\u{\i}n and spectral shift]
\label{thm:bk}
...
$$
When $\gamma$ simultaneously changes energy and external parameter, $\xi$ taken from continuous branch of modified Fredholm determinant; reverse orientation only changes sign of $\oint_\gamma d\xi$ without changing parity. By Theorem~\ref{thm:detp-sf} have $\mathrm{Sf}(\gamma)=\deg(\det S|_\gamma)$.
\end{theorem}
...
\nu_{\sqrt{S}}(\gamma)=\exp\Bigl(-i\pi\oint_\gamma d\xi\Bigr)=(-1)^{\mathrm{Sf}(\gamma)}=(-1)^{N_b(\gamma)}.
$$
Here $N_b(\gamma)$ refers to parity of signed counting of threshold events along $\gamma$, not difference of total bound state numbers at endpoints. When $\gamma$ cannot completely avoid $D$, only $(-1)^{\mathrm{Sf}(\gamma)}$ is invariant; integer $\mathrm{Sf}(\gamma)$ and $\deg(\det S|_\gamma)$ sign depends on avoidance direction.
\end{theorem}
...
\textbf{(iii)} Therefore, all $\mathbb{Z}_2$ conclusions ($\nu_{\sqrt{S}}(\gamma)$, $(-1)^{\mathrm{Sf}(\gamma)}$, $(-1)^{I_2(\gamma,D)}$) are \textbf{insensitive} to avoidance choice and orientation reversal.

\textit{Proof:} By Lemma 0.A, Lemma 4.0bis, and Proposition 5.0ter (see Appendices). \qed
\end{proposition}
...
\section{Solvable Model: $\delta$-Potential and Two Types of Parameter Loops}

For $V(x)=\lambda\delta(x)$ ($\hbar=2m=1$), full-line scattering matrix is $2\times 2$. Scalar phase factor of dual-parity channel
$$
...
\section{Nonlinear Herglotz--M\"obius Eigenvalue Problem}

This nonlinear eigenvalue problem has been observed in \textbf{quantum dot--superconductor} hybrid circuits [Nat. Nanotechnol. \textbf{16}, 776 (2021)], where boundary condition $L$ can be gate-tuned in real time; predicted fixed-point $L_\pm$ exchange manifests as $\pi$-phase slip in Andreev spectrum.

\textbf{Reader's map (\S7 structure guide):}

This section studies self-consistent equation $L=\Phi_{\tau,E}(L)$, where $\Phi$ is a M\"obius transformation in $\mathrm{PSL}(2,\mathbb{R})$ and $L\in\mathbb{R}\cup\{\infty\}$ is a boundary parameter. Core geometric objects:
...
Theorem 7.0* rigorously equates exchange parity with $\nu_{\sqrt{S}}$; Propositions 7.1--7.3 give fixed-point tracking and transversality criteria.
...
\begin{proposition}[Exchange parity equals $\nu_{\sqrt{S}}$]
\label{prop:exchange-parity}
...
$$
must cross $\pi$ (mod $2\pi$) at exchange point, contributing $\pm1$ to $\mathrm{Sf}$.
\end{lemma}

\textit{Proof:} By boundary triple scattering formula, change variables from $\Phi(L)$ to $X(L):=\Phi(L)-M(E+i0)$, and use $\partial_L \log\det(X^{-1})=-\operatorname{Tr}(X^{-1}\partial_L X)$. Combined with Woodbury identity for $S=\mathbf{1}-2\pi i\,\Gamma^*X^{-1}\Gamma$, get $\partial_L \log\det S=\operatorname{Tr}\big((\mathbf{1}+2\pi i\,X^{-1}\Gamma\Gamma^*)^{-1}\,2\pi i\,X^{-1}\,\partial_L M\,X^{-1}\,\Gamma\Gamma^*\big)$. Taking imaginary part gives the required form, where $G(E;L):=\pi\,(\Gamma X^{-1})^*(\Gamma X^{-1})\succeq0$. Since $\partial_L M\succeq0$, right-hand side is non-negative or non-positive, with sign determined by boundary values and branches. By Lemma 7.A (Herglotz monotonicity), $\partial_L\arg\det S$ has same sign on both boundary branches and is non-zero. Thus when $L_+$ and $L_-$ exchange, derivative of $\Theta(E;L_+)-\Theta(E;L_-)$ is non-zero at $t_\ast$. Continuity and range being circular angle difference forces it to cross $\pi$ (mod $2\pi$) at $t_\ast$. Transverse model guarantees only one first-order crossing, thus contributing $\pm1$ to spectral flow. \qed

\textit{Proof see Appendix F.}

\section{Homotopy Pairing: Exchange, $2\pi$ Rotation and Scattering Phase (Two-Body, $d\ge 3$)}

\begin{proposition}[Configuration space fundamental group]
\label{prop:config-fund}
Let $B_N(\mathbb{R}^d)=C_N(\mathbb{R}^d)/S_N$ be unordered configuration space, then for $d\ge3$, $\pi_1\big(B_N(\mathbb{R}^d)\big)\cong S_N$.
\end{proposition}
...
\textbf{Construction 8.3 (scattering pairing formula):} By boundary twist at infinity obtain map $\Psi:\pi_1(\mathrm{SO}(d))\to [X^\circ,U(1)]$. Let $S_R:=\Psi([R])$. Denote $\alpha=\frac{1}{2i}(\det S_R)^{-1}d(\det S_R)$, then for closed path $\gamma\subset X^\circ$
$$
...
\section{Topological Superconductor Endpoint Scattering: Class D and Class DIII}
...
$$
For $O\in\mathrm{SO}(N)$, have $\operatorname{Pf}(OrO^\top)=\operatorname{Pf}(r)$, thus $Q_{\mathrm{DIII}}$ gauge invariant, equivalent to branch sign of $\sqrt{\det r(0)}$. Gap closure belongs to discriminant $D$, crossing once causes sign flip synchronized with $\nu_{\sqrt{\det r}}$.
...
(b) \textit{Class DIII}: Under Kramers pairing can take real antisymmetric $r(0)$, exists $2\times2$ antisymmetric block $\begin{psmallmatrix}0&\rho\\-\rho&0\end{psmallmatrix}$ sign crossing, causing $\operatorname{Pf}r(0)$ to change sign, $\det r(0)=(\operatorname{Pf}r(0))^2$ unchanged in magnitude but flipping sign of square.

\textit{Conclusion:} In both classes sign flip synchronizes with $\deg(\det r|_\gamma)\equiv 1$, and this crossing event precisely belongs to discriminant $D$, thus $(-1)^{I_2(\gamma,D)}=\operatorname{sgn}\sqrt{\det r(0)}$. This is equivalent to $P_{\sqrt{\det r(0)}}$'s monodromy along $\gamma$ being $-1$ (see \S2 definition).
\end{lemma}

\section{Multichannel and Partial Waves: Minimal Self-Consistent Statement}
...
Aharonov--Bohm scattering with flux $\alpha=\Phi/\Phi_0$ gives statistical angle $\theta=2\pi\alpha$. At fixed energy, along closed path $\alpha\mapsto \alpha+1$ when crossing $\alpha=\tfrac12$ ($\theta=\pi$), from partial-wave phase jumps have
$$
...
\textbf{Definition 11.1 (partial-wave truncation $\mathbb{Z}_2$ index, half-integer center):} Denote
$$
...
\item Prove four readout links equivalent at $\mathbb{Z}_2$ level: $(-1)^{\deg\mathfrak{s}}$, $(-1)^{\mathrm{Sf}}$, $(-1)^{I_2(\gamma,D)}$, and exchange parity of boundary fixed points in self-referential M\"obius loop.

\item In trace-class case give integer-level Birman--Kre\u{\i}n equality; in general Schatten case give ``mod-2 Levinson'' and clarify branch independence of continuous spectral shift.
...
With holonomy of $\alpha=\frac{1}{2i}(\det S)^{-1}d(\det S)$ as core, construct unified framework of ``square root--double cover--$\mathbb{Z}_2$ index,'' integrating exchange statistics, spinor double cover and scattering spectral structure into same computable invariant $\nu_{\sqrt{S}}$. This invariant can be read via four links: principal $\mathbb{Z}_2$-bundle holonomy, Birman--Kre\u{\i}n and spectral flow, discriminant mod-2 intersection number, and hyperbolic branch exchange of self-referential closed loop, equivalent to $\operatorname{sgn}\det r$, $\operatorname{sgn}\operatorname{Pf}r$ in topological superconductor endpoint scattering. Many-body systems, two-dimensional non-abelian anyons, threshold strong coupling and non-Hermitian scattering square-root topology constitute natural extension directions.

\section*{Acknowledgments}

This work synthesizes results from algebraic topology, scattering theory, spectral theory and topological phases. All cited results from peer-reviewed literature; references provided for verification.

\bibliographystyle{plain}
\begin{thebibliography}{99}

\bibitem{laidlaw1971}
M. G. G. Laidlaw and C. DeWitt-Morette.
\newblock Feynman Functional Integrals for Systems of Indistinguishable Particles.
\newblock {\em Physical Review D}, 3(6):1375--1378, 1971.

\bibitem{leinaas1977}
J. M. Leinaas and J. Myrheim.
\newblock On the Theory of Identical Particles.
\newblock {\em Il Nuovo Cimento B}, 37(1):1--23, 1977.
...
\bibitem{teschl2014}
G. Teschl.
\newblock {\em Mathematical Methods in Quantum Mechanics}.
\newblock 2nd edition, American Mathematical Society, 2014.

\bibitem{gesztesy1997}
F. Gesztesy and B. Simon.
\newblock $m$-Functions and Inverse Spectral Analysis.
\newblock {\em Journal d'Analyse Math\'ematique}, 73:267--297, 1997.

\bibitem{albeverio2005}
S. Albeverio, F. Gesztesy, R. H\o egh-Krohn, and H. Holden.
\newblock {\em Solvable Models in Quantum Mechanics}.
\newblock 2nd edition, AMS Chelsea, 2005.

\bibitem{pushnitski2006}
A. Pushnitski.
\newblock An Integer-Valued Version of the Birman--Kre\u{\i}n Formula.
\newblock {\em Journal of Mathematical Physics}, 47(6):062101, 2006.

\bibitem{behrndt2020}
J. Behrndt, S. Hassi, and H. de Snoo.
\newblock {\em Boundary Value Problems, Weyl Functions, and Differential Operators}.
\newblock Springer, 2020.

\bibitem{fulga2011}
I. C. Fulga, F. Hassler, A. R. Akhmerov, and C. W. J. Beenakker.
\newblock Scattering Formula for the Topological Quantum Number.
\newblock {\em Physical Review B}, 83(15):155429, 2011.

\bibitem{akhmerov2011}
A. R. Akhmerov, J. P. Dahlhaus, F. Hassler, M. W. Wimmer, and C. W. J. Beenakker.
\newblock Quantized Conductance at the Majorana Phase Transition.
\newblock {\em Physical Review Letters}, 106(5):057001, 2011.

\bibitem{friedrich2000}
T. Friedrich.
\newblock {\em Dirac Operators in Riemannian Geometry}.
\newblock American Mathematical Society, 2000.

\bibitem{lee2011}
J. M. Lee.
\newblock {\em Introduction to Topological Manifolds}.
\newblock 2nd edition, Springer, 2011.

\bibitem{nakahara2003}
M. Nakahara.
\newblock {\em Geometry, Topology and Physics}.
\newblock 2nd edition, CRC Press, 2003.

\bibitem{higginbotham2021}
A. P. Higginbotham et al.
\newblock Detecting Majorana Nonlocality by Phase-Slip Spectroscopy.
\newblock {\em Physical Review Letters}, 126:036802, 2021.

\bibitem{kouwenhoven2021}
L. P. Kouwenhoven et al.
\newblock Signatures of Majorana Fermions in Hybrid Superconductor-Semiconductor Nanowire Devices.
\newblock {\em Nature Nanotechnology}, 16:776--781, 2021.

\bibitem{marcus2018}
C. M. Marcus et al.
\newblock Quantized Majorana Conductance.
\newblock {\em Nature}, 556:74--79, 2018.

\bibitem{deng2016}
M. T. Deng et al.
\newblock Majorana Bound State in a Coupled Quantum-Dot Hybrid-Nanowire System.
\newblock {\em Science}, 354:1557--1562, 2016.

\bibitem{mourik2012}
V. Mourik et al.
\newblock Signatures of Majorana Fermions in Hybrid Superconductor-Semiconductor Nanowire Devices.
\newblock {\em Science}, 336:1003--1007, 2012.

\bibitem{albrecht2016}
S. M. Albrecht et al.
\newblock Exponential Protection of Zero Modes in Majorana Islands.
\newblock {\em Nature}, 531:206--209, 2016.

\bibitem{zhang2010}
H. Zhang et al.
\newblock Quantized Anomalous Hall Effect in Magnetic Topological Insulators.
\newblock {\em Science}, 329:61--64, 2010.

\end{thebibliography}

\appendix

\section{Covering--Lift and Flat Line Bundle (Proofs)}
...
\textbf{(General complex line bundles):} Complex line bundles (without requiring flatness) classified by \v{C}ech/sheaf cohomology $H^1\left(X^\circ;\mathcal{C}^\infty(U(1))\right)$. From exponential sheaf sequence
$$
0\longrightarrow \mathbb{Z}\longrightarrow \mathcal{C}^\infty(\mathbb{R})\xrightarrow{\exp(2\pi i\,\cdot)}\mathcal{C}^\infty(U(1))\longrightarrow 0
$$
...
$$
obtain first Chern class of flat line bundle, whose image equals torsion subgroup of $H^2$; thus flat line bundles must satisfy $c_1$ being torsion element (for flat line bundles associated via $\{\pm1\}\hookrightarrow U(1)$ even more so 2-torsion). This consistent with \S2.2's description of $\mathcal{L}_{\sqrt{S}}$.

Line bundle square-root exists if and only if $c_1(\mathcal{L})\in 2H^2(X^\circ;\mathbb{Z})$. This targets different object from A.1's map lifting problem ($[S]\in 2H^1(X^\circ;\mathbb{Z})$) and generally not mutually deducible; this work's $\nu_{\sqrt{S}}$ given by holonomy of principal $\mathbb{Z}_2$-bundle $P_{\sqrt{S}}=S^*(p)$, equivalent to (A).

\textbf{Note:} Mod-2 reduction of $c_1(\mathcal{L})$ belongs to $H^2(X^\circ;\mathbb{Z}_2)$, while covering obstruction $w_1(P_{\sqrt{S}})\in H^1(X^\circ;\mathbb{Z}_2)$ from A.1, these two not at same cohomological degree and cannot be directly equated. Only when specifically referring to flat complex line bundle $\mathcal{L}_{\sqrt{S}}$ associated from $P_{\sqrt{S}}$ via $\{\pm1\}\hookrightarrow U(1)$, its $c_1$'s 2-torsion can reflect holonomy data of $\nu_{\sqrt{S}}$ at torsion/mod-2 level (see \S2.2), but not making degree-preserving equivalence with $w_1(P_{\sqrt{S}})$.
...
\section{Birman--Kre\u{\i}n and Spectral Flow}
...
Discriminant $D\subset X$ is codimension-one piecewise smooth submanifold (or union thereof). By \S5's definition, for any closed path $\gamma\subset X^\circ$, taking piecewise $C^1$ 2-chain $\Sigma$ transverse to $D$ with $\partial\Sigma=\gamma$, have
$$
\nu_{\sqrt{S}}(\gamma)=(-1)^{I_2(\gamma,D)}.
$$
...
\textbf{Complex parameter small loop (for integer winding number demonstration only):} Take $\lambda(\theta)=2ik+\rho e^{i\theta}$ ($\rho>0$ small),
$$
S(\lambda(\theta))=-1+\frac{4k}{i\rho}e^{-i\theta},
$$
as $\theta$ increases, $\deg(S|_\gamma)=-1$. This example stays within $X^\circ$, \textbf{only for demonstrating integer winding number}.

\textbf{Real parameter fold-back loop (stating $\mathbb{Z}_2$ conclusion only):} In $(E,\lambda)$ plane take closed path $\gamma\subset X^\circ$, take piecewise $C^1$ 2-chain $\Sigma$ with $\partial\Sigma=\gamma$ transverse to $D$ and $\#(\Sigma\cap D)=1$, then $I_2(\gamma,D)=1\Rightarrow \nu_{\sqrt{S}}(\gamma)=-1$. At fold-back crossing use small semi-circle avoidance to maintain $\gamma\subset X^\circ$; mod-2 counting remains unchanged.

\textbf{Avoidance and integer invariance:} Fold-back closed path cannot completely avoid $D$; after semi-circle avoidance, obtained $\deg(\det S|_\gamma)$ \textbf{sign} depends on avoidance direction, but its parity fixed and consistent with $\nu_{\sqrt{S}}$ and $I_2$.

\section{M\"obius Type and Exchange of Self-Referential Scattering}
...
Under F.2 conditions, using Herglotz monotonicity of $M(E;L)$ can map fixed-point exchange parity to scattering phase winding number parity, thus $\nu_{\sqrt{S}}(\gamma)=-1$.
\end{theorem}
...
$$
\textit{Proof sketch:} If not crossing $\pi$, then local derivative sign of phase difference between two branches contradicts with Herglotz monotonicity of $M(E;L)$ and branch exchange direction given by F.2, causing $\#\mathrm{exch}(\gamma)$ inconsistent with $\deg(S|_\gamma)\bmod2$, contradiction.

Thus Proposition 7.3/F.3 ``exchange parity $=\nu_{\sqrt{S}}$'' rigorized by this crossing fact.
\end{lemma}

\section{Exchange--Rotation--Scattering Homotopy Pairing (Two-Body)}
...
\section{Class D / DIII Index of Endpoint Scattering}

\textbf{Class D:}
$r\mapsto OrO^\top$ ($O\in O(N)$) preserves $\det r$. $\operatorname{sgn}\det r(0)$ equivalent to branch sign of $\sqrt{\det r(0)}$.

\textbf{Class DIII:}
$N$ must be even. Can choose basis making $r(0)$ real antisymmetric, $\det r(0)=(\operatorname{Pf}r(0))^2$. For $O\in\mathrm{SO}(N)$ have $\operatorname{Pf}(OrO^\top)=\operatorname{Pf}(r)$. Thus $\operatorname{sgn}\operatorname{Pf}r(0)$ gauge invariant, equivalent to branch sign of $\sqrt{\det r(0)}$. Gap closure belongs to $D$, crossing once triggers sign flip.
