...
% Title information
\title{Self-Referential Scattering and Fermion Genesis:\\
Riccati Square Roots, Spinor Double Covers, and $\mathbb{Z}_2$ Exchange Phase}
\author{Haobo Ma$^1$ \and Wenlin Zhang$^2$\\
\small $^1$Independent Researcher\\
\small $^2$National University of Singapore}

\date{Version 2.15 --- \today}
...
\begin{abstract}
On parameter space $X^\circ\subset X$ after removing discriminant, consider phase exponential map $S:X^\circ\to U(1)$ of fixed-energy scattering. Along square cover $p:U(1)\to U(1)$, $p(z)=z^2$, the pullback
$$
...
\textbf{Experimental prediction:} In gate-tunable Josephson junctions, when Andreev channel number $\lesssim 4$, perform single $2\pi$ scan of superconducting phase difference $\phi$ at zero energy bias; each time crossing Majorana crossing event, the \textbf{dimensionless $\mathbb{Z}_2$ index}
$$
...
\deg(S|_C)=-\sum_j m_j
$$
in \S3 is \textbf{momentum $(k)$ plane spectral loop} $C$ analytic counting, used for spectral structure analysis of $S=f(-k)/f(k)$; it does \textbf{not} identify and compare with parameter loop $\gamma$ at integer level. This work does \textbf{not} claim integer equalities like $\deg(\det S|_\gamma)=\deg(S|_C)$ or $\deg_\lambda=\deg_k$. Main Theorem~\ref{thm:main} only asserts
$$
(-1)^{\deg(\det S|_\gamma)}=(-1)^{\mathrm{Sf}(\gamma)}=(-1)^{N_b(\gamma)}=(-1)^{I_2(\gamma,D)}.
$$
(See \S3 ``Note (spectral loop vs parameter loop)''.)
...
\begin{theorem}[Unified equivalence]
\label{thm:main}
Under Section 0 assumptions, for any closed path $\gamma\subset X^\circ$ have
$$
\nu_{\sqrt{S}}(\gamma)
...
=(-1)^{I_2(\gamma,D)}.
$$
where for \textbf{multichannel/partial wave cases} unified substitution uses overall phase exponential $\det S$ (when necessary use modified determinant $\det_p S$); for \textbf{single channel} have $\det S=S$.
...
\textit{Conclusion:} In both classes sign flip synchronizes with $\deg(\det r|_\gamma)\equiv 1$, and this crossing event precisely belongs to discriminant $D$, thus $(-1)^{I_2(\gamma,D)}=\operatorname{sgn}\sqrt{\det r(0)}$.
\end{lemma}
...
$$
When partial-wave index infinite, this work \textbf{fixes adoption} of modified determinant $\det_2$ to define overall phase $\det_2 S$, and accordingly read $\mathbb{Z}_2$ image of $\nu_{\sqrt{\det_2 S}}(\gamma)$. For Aharonov--Bohm model, partial-wave truncation finite product and $\det_2$ give consistent \textbf{mod-2} result when crossing $\alpha=\tfrac12$; this work does not claim universal independence of \textbf{all} regularization schemes here. General $\theta\neq 0,\pi$ beyond this work's $\mathbb{Z}_2$ framework, this work only captures its mod-2 projection.
