...
**版本**：v7.0（Part I：核心定理与实现要则；附录含主要证明链）
...
**（0.5）双时间分离。** 最早到达时间 $t_*$ 确定因果前沿；群延迟刻度 $T_\gamma=(2\pi)^{-1}\operatorname{tr}\mathsf Q$ 为操作读数，可能取负且不与 $t_*$ 直接比较。卷积支撑的 Titchmarsh 等式给出前沿次可加性。
...
取时间窗 $w(t)$、频核 $h(\omega)$，以母尺测度加权，线性响应的窗化读数为
$$
\langle \Psi\rangle_{w,h}\;=\;\sum_{j=1}^J \operatorname{Res}\bigl(\widehat\Psi(\omega),\omega_j\bigr)\,\widehat w(\omega_j)\,\widehat h(\omega_j)\;+\;\text{Tail},
$$
其中 $\{\omega_j\}$ 为有限多主导 QNM 极点；Tail 由 Price 定律控制的幂律散射尾给出（Schwarzschild 上点态衰减已得精确次幂）。
...
**定理 E（QNM 残数—尾项的窗化展开）。** 设有限能窗与频核满足 Schwartz 正则性，则窗化响应等于主导 QNM 极点的有限和加上由散射尾控制的幂律项；Schwarzschild 情形尾项次幂已得到点态精确估计。
...
**A.1 单向支撑与前沿。** 若频域边值属于 $H^2(\mathbb C^+)$，则时域响应因果；Titchmarsh 卷积支撑定理给出 $\inf\supp(f*g)=\inf\supp f+\inf\supp g$，从而最早到达 $t_*$ 的次可加性成立。
...
$$
取 $w=z=x\in\mathbb R$ 的对角极限，并用 $\tfrac{d}{dx}\arg E(x)=-\Im\frac{E'}{E}(x)=\varphi'(x)$，得 $K(x,x)=\pi^{-1}\varphi'(x)|E(x)|^2$。
