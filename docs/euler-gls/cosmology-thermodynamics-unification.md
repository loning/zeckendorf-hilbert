...
写成 $r_A^{-2}=H^2+\kappa/a^2=\tfrac{8\pi G}{3}\rho-\kappa/a^2+\tfrac{\Lambda}{3}$，对时间微分并代入 $\dot\rho=-3H(\rho+p)$，再乘以 $T$ 与 $dS_h/dt$ 的因子，可得
$$
T\,\dot S_h
=4\pi H r_A^3(\rho+p)\;-\;\frac{\kappa H r_A^3}{G a^2}\;+\;\frac{r_A^3}{6G}\,\dot\Lambda.
\tag{1}
$$
右端三项依次诠释为：物质/辐射的焓通量熵、几何曲率交换熵、真空曲率熵；当 $\dot\Lambda=0$ 时，真空项不产生局域熵通量，只通过几何刻度 $(T,A)$ 调制可逆熵。([arXiv][2])
...
Raychaudhuri 方程给出测地线族膨胀的聚焦/发散演化；在能量条件与适当边界下，表观/事件视界面积的单调性与宇宙学广义第二定律（Q-screen 形式）共同保证沿自然时间箭头的广义熵 $S_{\rm gen}$ 不减。式 $(1)$ 提供了视界上一阶"熵收支"的几何分拆，宇宙学演化可等价描述为视界几何熵的产生—交换—调制的动力学。([物理评论链接管理器][3])
...
在 de Sitter 极限 $H^2=\Lambda/3$ 中，温度与熵分别为 $T_{\rm dS}=H/(2\pi)$、$S_{\rm dS}=A/(4G)=3\pi/(G\Lambda)$，体现真空曲率对几何刻度的可逆设定；若 $\Lambda$ 随时间缓变或含高阶曲率修正，则 $\Phi_\Lambda\propto \dot\Lambda$ 与内禀熵产生 $d_iS$ 同时出现。([物理评论链接管理器][5])
...
**定理 2（第一方程的熵平衡分拆）**：对 $r_A^{-2}=\tfrac{8\pi G}{3}\rho-\kappa/a^2+\tfrac{\Lambda}{3}$ 微分并使用 $\dot\rho=-3H(\rho+p)$，在表观视界上成立
$$
T\,\dot S_h=\underbrace{4\pi H r_A^3(\rho+p)}_{\Phi_{\rm m}}
\;+\;\underbrace{\big(-\kappa H r_A^3/(G a^2)\big)}_{\Phi_\kappa}
\;+\;\underbrace{\big(r_A^3\dot\Lambda/(6G)\big)}_{\Phi_\Lambda}.
$$
其中 $\Phi_{\rm m}$ 为焓通量熵，$\Phi_\kappa$ 为曲率几何交换熵，$\Phi_\Lambda$ 为真空曲率熵（$\dot\Lambda=0$ 时为零）。([arXiv][2])
...
（1）晚期几何刻度：$\Lambda$ 主导时代 $T=H/(2\pi)$、$S_h=3\pi/(G\Lambda)$。运行真空模型的缓变 $\dot\Lambda$ 将以 $\Phi_\Lambda$ 的形式进入熵平衡并影响 $\dot S_{\rm gen}$。([物理评论链接管理器][5])
...
（3）曲率交换熵：若 $|\Omega_k|\neq 0$，$\Phi_\kappa$ 在再电离前后给出可测的几何交换项符号，其现值受 BAO/CMB 对 $\Omega_k$ 的约束；相应地，$\Phi_\kappa$ 的贡献比例随 $a(t)$ 的演化而调制。([arXiv][2])
...
Jacobson 将 Einstein 方程视作局域 Rindler 视界族上的状态方程 $\delta Q=T\,dS$；本文在宇宙学尺度以表观视界的几何刻度 $(S_h,T)$ 将该思想具体化，给出 Friedmann 方程的熵学分拆。含高阶曲率/非最小耦合时，需引入 $d_iS$ 的非平衡扩展；在 de Sitter 极限，暗能量作为真空几何曲率仅改变可逆几何熵，而不在局域产生熵通量，除非 $\Lambda$ 时间依赖。([物理评论链接管理器][1])
...
对 $r_A^{-2}=\tfrac{8\pi G}{3}\rho-\kappa/a^2+\tfrac{\Lambda}{3}$ 微分，得 $-2r_A^{-3}\dot r_A=\tfrac{8\pi G}{3}\dot\rho+2(\kappa/a^2)H-\tfrac{1}{3}\dot\Lambda$。两侧乘 $-\tfrac{\pi r_A^4}{G}$ 得 $\dot S_h=-\tfrac{8\pi^2 r_A^4}{3}\dot\rho-\tfrac{2\pi r_A^4}{G}\tfrac{\kappa H}{a^2}+\tfrac{\pi r_A^4}{3G}\tfrac{\dot\Lambda}{2}$。再乘 $T=(2\pi r_A)^{-1}$ 并以 $\dot\rho=-3H(\rho+p)$，化为 $T\,\dot S_h=4\pi H r_A^3(\rho+p)-\tfrac{\kappa H r_A^3}{G a^2}+\tfrac{r_A^3}{6G}\dot\Lambda$。([arXiv][2])
