...
**Version: 1.3**（2025-10-30，Asia/Dubai）
...
* **推迟格林函数（3D 波/Maxwell）**：$\square G_{\rm ret}=\delta$，其显式式为 $G_{\rm ret}(t,\mathbf r)=\delta\!\bigl(t-|\mathbf r|/c\bigr)/(4\pi|\mathbf r|)$，支撑恰在 $t=r/c$。([维基百科][4])

* **光学度量（Gordon）**：各向同性介质可等效为**光学度量** $g^{\rm opt}_{\mu\nu}$；前沿速度保持 $c$（Sommerfeld–Brillouin），群速可异于 $c$。现代综述与推广见 Leonhardt–Philbin 及后续工作。([arXiv][5])
...
* **de Branges 核对角**：对 Hermite–Biehler 函数 $E$ 的空间 $\mathcal H(E)$，有 $K(x,x)=\tfrac{1}{\pi}\,\varphi'(x)\,|E(x)|^2$，其中 $\varphi$ 为相位函数。
...
**(A5) 前沿—信息一致**：前沿与信息速度上确界同为 $c$。([维基百科][4])

**(A6) 采样—误差闭合**：读数误差按 NPE 三分解给出非渐近上界。([webusers.imj-prg.fr][7])

**(A7) SI 对齐**：$c$ 取 SI 固定值；"以延迟计长"与"以时定长"互逆实现（§4、§8）。

**(A8) 相位密度几何**：de Branges 核对角 $K(x,x)=|E|^2\varphi'/\pi$ 诱导 $\mu_\varphi:=(\varphi'/\pi)\,dx$ 作为窗化刻度。
...
**定义 2.2（空间）** 真空中以**雷达距**定义空间距离：$d(A,B):=\tfrac{c}{2}\,\mathsf T_{\rm roundtrip}(A\to B\to A)$。介质中以**Gordon 光学度量** $g^{\rm opt}$ 的零测地线定义几何光程；**前沿速度仍为 $c$**。([arXiv][9])

**定义 2.3（同时切片）** 选参考世界线与往返协议，令 $\Sigma_t:=\{e\in\mathcal E:\ \mathsf T_{\rm roundtrip}=2t\}$，其三维度量由雷达距或 $g^{\rm opt}$ 诱导。
...
(1) **度量—读数一致**：真空链路 $L$ 上 $\mathsf T=L/c$；介质零测地线与前沿一致；
...
**定理 5.1（因果前沿 = $c$）** LTI 严格因果信道的最早非零响应时间满足 $t_{\min}(L)=L/c$。

**证明**：令源信号 $x$ 支持于 $t\ge 0$。距离 $L$ 处的输出
$$
y(t)=\int_{\mathbb R} G_{\rm ret}(t-\tau,L)\,x(\tau)\,d\tau.
$$
真空三维波（含 Maxwell）有
$$
G_{\rm ret}(t,r)=\frac{\delta(t-r/c)}{4\pi r}+H(t-r/c)\,g(t,r),
$$
其中 $H$ 为阶跃函数、$g$ 为 $t\ge r/c$ 的局部可积核。因此当 $t<L/c$ 时，对任意 $\tau\ge 0$，有 $t-\tau<L/c\Rightarrow G_{\rm ret}(t-\tau,L)=0$，从而 $y(t)=0$。取 $x=\delta$ 可见 $y(t)=\delta(t-L/c)/(4\pi L)+\cdots$，故最早非零响应恰在 $t=L/c$。这在频域与 KK 等价：严格因果 $\Leftrightarrow$ 上半平面解析；由移位定理，最小支撑端点即传播前沿 $L/c$。$\square$

**定理 5.2（信息光锥 = $c$）** 设接收端在 $\Delta t$ 前仅含与发送独立的噪声，无预共享携信变量。记门限互信息的首次可检时间 $T_\delta(L)$。则 $c_{\rm info}:=\lim_{\delta\downarrow0}\sup_{L>0}L/T_\delta(L)=c$。

**证明**：设输入过程 $X$ 支持于 $t\ge 0$，噪声 $N$ 与 $X$ 独立，接收端观测
$$
Y_t := \{Y(s): 0\le s\le t\},\qquad Y=G_{\rm ret}\!*\!X+N.
$$

**(1) 零互信息**：若 $t<L/c$，则对一切 $s\le t$ 与 $\tau\ge 0$ 有 $G_{\rm ret}(s-\tau,L)=0$，故 $Y(s)=N(s)$。独立噪声的可测函数仍独立，因而 $X\perp Y_t\Rightarrow I(X;Y_t)=0$。

**(2) 正互信息与门限时间**：任取 $\varepsilon>0$，令 $t_\varepsilon=L/c+\varepsilon$。取标量试探 $X_\alpha(\tau)=\alpha\,\psi(\tau)$（$\psi$ 为单位能量的短时脉冲）。则
$$
Z_\varepsilon:=\int_0^{t_\varepsilon}G_{\rm ret}(t_\varepsilon-\tau,L)\,\psi(\tau)\,d\tau
$$
非零（由 $G_{\rm ret}$ 在 $t>L/c$ 的尾支撑）。于是
$$
Y(t_\varepsilon)=\alpha\,Z_\varepsilon+N(t_\varepsilon).
$$
在高斯噪声下，二元输入（$X\in\{\pm\alpha\}$ 等概）给出
$$
I\bigl(X;Y(t_\varepsilon)\bigr)\ \ge\ \tfrac{1}{2}\log\!\bigl(1+\alpha^2|Z_\varepsilon|^2/\sigma^2\bigr)
=\Theta(\alpha^2),
$$
故对任意小的 $\delta>0$ 可取充分小的 $\alpha$ 使 $I\ge \delta$。定义 $T_\delta(L)=\inf\{t:I(X;Y_{[0,t]})\ge\delta\}$，便得
$$
\forall \varepsilon>0,\ \exists \delta(\varepsilon)\downarrow 0:\quad T_\delta(L)\le L/c+\varepsilon,
$$
从而 $c_{\rm info}:=\lim_{\delta\downarrow0}\sup_{L>0}\!L/T_\delta(L)=c$。$\square$

---

## 6. 相位密度几何与迹公式

**命题 6.1** 在 de Branges 空间 $\mathcal H(E)$ 上，几乎处处 $K(x,x)=\tfrac{1}{\pi}\,\varphi'(x)\,|E(x)|^2$。因此**相位密度** $\rho(x):=\varphi'(x)/\pi$ 给出自然测度，窗化迹公式写成 $\mathrm{Tr}(M_{w_R})=\int w_R(x)\,\rho(x)\,dx$。该恒等式为"相位—密度—延迟"在 RKHS 的实现。

**证明**：设 $E$ 为 Hermite–Biehler 函数，$E^#(z):=\overline{E(\overline z)}$。de Branges 空间 $\mathcal H(E)$ 的再生核为
$$
K(z,w)=\frac{\overline{E(w)}\,E(z)-\overline{E^#(w)}\,E^#(z)}{2\pi i\,(\overline w-z)}.
$$
取 $w=z=x\in\mathbb R$，用洛必达法则并记 $E(x)=|E(x)|e^{-i\varphi(x)}$ 得
$$
K(x,x)=\frac{1}{2\pi i}\Big(\overline{E(x)}E'(x)-\overline{E'(x)}E(x)\Big)
...
半径 $r$ 的 RCA 在 $t$ 步的影响域为 $\pm r t$，等效"离散光锥"，离散"光速" $c_{\rm disc}=r a/\Delta t$。**Curtis–Hedlund–Lyndon 定理**刻画了"连续且与移位可换 $\Leftrightarrow$ 滑动块码"，并保证可逆性时逆演化亦为 CA，从而实现可逆因果传播。([SpringerLink][15])
...
* **空间**：通过等时往返读数选定的**同时切片 $\Sigma_t$** 及其由雷达距/光学度量诱导的三维度量；前沿 $=c$ 保证与因果一致。
