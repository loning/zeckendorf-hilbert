...
其中 $\rho=\pi^{-1}\Im m(\cdot+i0)$，$\phi_0$ 为几何主项密度；$\rho$ 与 $\phi_0$ 视为与 $w$ 无关的背景密度，进入 $\mathfrak E$ 仅线性作用于 $w_R$。取可检范数
...
*要点*. $\mathsf{PW}_\Omega^{\mathrm{even}}$ 为闭线性子空间；二次目标严格凸且强 coercive。由时频限制算子谱存在 $\lambda_0(\Omega/R,T)<1$ 使
$|\mathbf 1_{|t|>T}w_R|_{L^2}^2\ge(1-\lambda_0)|w_R|_{L^2}^2$。配合导数项得强 coercivity；约束 $w(0)=1$ 排除零解。

### 2.3 一阶必要条件：弱式与频域强式

**弱式.** 对任意 $\varphi\in\mathsf{PW}_\Omega^{\mathrm{even}}$，
...
等价于 $\mathbf P_{\Omega}^{\mathrm{even}}\big[\sum_{j}\gamma_j\,\partial_t^{4j}w_R^\star
...
使得对任意 $\varphi\in\mathsf{PW}_\Omega^{\mathrm{even}}$，
