...
* **POVM-化读数**：柔叶 $\{M_j\}_{j=1}^n$ 非负且 $\sum_j M_j=\mathbf1$。令 $E_j=\Psi^{1/2}M_j\Psi^{1/2}\succeq0$，$\sum_jE_j=\Psi$。Born-型概率
...
(C0g) $\phi$ 连续有界，谱 $\sigma(L)\subset[0,\lambda_{\max}]$；(C1g) 相关和/内积可由 $\ell^1/\ell^2$ 主导；(C2g) 切比近似尾项服从**Bernstein 椭圆**界 $\lesssim C_\phi\rho^{-m}$；(C3g) $(S,\Omega)$ 构成带限帧；(C4g) 提交约束集仿射线性且 KL 在单纯形上严格凸；(C5g) $\|W\|_{\infty}<\infty$ 保证 $\Psi$ 有界且 $H^\ast\Psi H$ 有定义。
...
设 $q$ 满支撑，矩阵 $A=[a_{\ell j}]$ 使 $\mathcal C(X)$ 非空且与单纯形内点相交。记
...
定义代价 $\mathfrak E(X;W,H)=\langle HX,\Psi HX\rangle=\langle X, H^\ast\Psi H\,X\rangle$，在 $\|X\|=1$ 上极小化。则极小点当且仅当解广义本征问题 $H^\ast\Psi H\,X=\lambda X$ 且取**最小本征模**（含简并自由）。

**证明**。Rayleigh–Ritz 变分原理：$\min_{\|X\|=1}\langle X, H^\ast\Psi H\,X\rangle$ 在 $H^\ast\Psi H$ 的最小本征空间取得；必要性与充分性来自 Hermitian 情形的极值表征。∎

**注记**。当 $H=\phi(L)$ 且 $\Psi\succeq0$ 时，$H^\ast\Psi H$ 自伴 $\succeq0$。最小本征空间对有界扰动稳定，其夹角满足 Davis–Kahan 型上界。

**推论 4.2（扰动稳定性）** 设 $A=H^\ast\Psi H$。若 $\|E\|_2\le\varepsilon$，则 $|\lambda_{\min}(A+E)-\lambda_{\min}(A)|\le \varepsilon$（Weyl 不等式）；最小本征子空间与原子空间的夹角由 Davis–Kahan 型界控制。

---

### 定理 G3（Windows = 极大极小最优，充要）

设目标带宽 $[0,\Omega]$ 与切比次数 $m$。定义最坏情形误差

$$
\mathcal E(W,\phi,m;S)\coloneqq
\underbrace{\sup_{\lambda>\Omega}|\phi(\lambda)|}_{\text{谱泄露}}
+\underbrace{\kappa(S,\Omega)}_{\text{帧误差}}
+\underbrace{C_\phi\,\rho^{-m}}_{\text{切比尾项}} ,
$$

其中 $\kappa(S,\Omega)=\sqrt{B/A}-1$，$\rho>1$ 为目标函数在 Bernstein 椭圆内的解析半径。则存在最优 $(W^\ast,\phi^\ast,m^\ast,S^\ast)$ 使$^*$

> $^*$ 当参数集取为紧集（如固定 $m$ 的上界、窗幅度与支撑有界、采样集在有限集合上取值）时，最优点存在；否则存在下确界与极小序列。
...
且对**固定** $S,\Omega,m$ 的幅度响应整形子问题，$\phi^\ast$ 的**等波纹（交错等幅）**性质是**必要且充要**的（Remez / Chebyshev 交错定理）；在**联合**优化 $W,\phi,m,S$ 的情形，等波纹给出**$\phi$-子问题**的一阶最优性证据，总体最优还受 $\kappa(S,\Omega)$ 与 $\rho^{-m}$ 的权衡影响。
...
对任意 $X\in\mathcal B_\Omega$，以 $H_m$ 近似 $H$，并在采样集 $S$ 上读数与重构。令 $\mathfrak E(X;W,H)=\langle HX,\Psi HX\rangle$。则存在常数 $C_\phi,\rho>1$ 使

$$
\big|\ \mathfrak E(X;W,H_m)-\mathfrak E(X;W,H)\ \big|
\ \le
\underbrace{\sup_{\lambda>\Omega}|\phi(\lambda)|}_{\mathrm{Leak}(\phi,\Omega)}
+\underbrace{\kappa(S,\Omega)\,\|X\|_2^2}_{\text{帧误差}}
+\underbrace{C_\phi\,\rho^{-m}}_{\text{切比尾项}} .
$$

若某读数是 $\mathfrak E$ 的 1-Lipschitz 函数（如加权归一化），同阶界亦成立。

**证明**。三角分解为阻带抑制项、带限帧重构项与切比尾项；三项分别由滤波器外带上界、$(A,B)$ 帧界与 Bernstein 椭圆近似定理控制。∎

---

## 5. 资源受限的可判性（下界）

考虑二元检验 $(H_0,H_1)$，其叶分布在总变差上分离：$\inf_{P\in H_0,Q\in H_1}\mathrm{TV}(P,Q)\ge r$。在显著性 $\alpha$、功效 $1-\beta$ 下，任意流程所需样本数满足

$$
N\ \ge\ \frac{\log\!\big(\tfrac{1}{2(\alpha+\beta)}\big)}{2\,\mathrm{TV}(P,Q)^2},
$$

等价地，若以 $\mathrm{KL}$ 表示分离度，则 $N\ge \tfrac{\log\!\big(\tfrac{1}{2(\alpha+\beta)}\big)}{\mathrm{KL}(P\|Q)}$。

**证明提纲**。Le Cam 两点法把下界化为二分布检验；Pinsker 不等式（自然对数）给出 $\mathrm{KL}(P\|Q)\ge 2\,\mathrm{TV}(P,Q)^2$；张量化 $\mathrm{KL}(P^{\otimes N}\|Q^{\otimes N})=N\,\mathrm{KL}(P\|Q)$ 与 Neyman–Pearson 最优性结合得到所述下界。∎
