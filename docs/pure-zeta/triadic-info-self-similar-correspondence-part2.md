...
3. 比例因子$(i_+ / (1/\phi^2))$量化临界线统计与Zeckendorf基准的偏差：
   $$
   \frac{i_+}{1/\phi^2} = \frac{0.403}{0.382} \approx 1.055
   $$
4. 分形指数$D_f$将线性修正放大为非线性缩放：
   $$
   (1.055)^{1.44042} \approx 1.0805
   $$
5. 故$S_{BH}^{\text{fractal}} \approx 1.0805 \times S_{BH}$，修正约$+8.05\%$。
...
其中$S_\Omega = -(1/3)\ln(1/3) - (2/3)\ln(2/3) \approx 0.6365$，$S_{max} = \ln 3 \approx 1.0986$。故：

$$
S_{BH}^{\Omega} \approx 0.5795 \times S_{BH}
$$

这给出负修正$-42.05\%$，对应反对称性导致的信息抑制。
...
| Planck质量        | $2.18 \times 10^{-8}$ | $1$ | $0.25$ | $0.275$ | $+10.1\%$ | 量子引力阈值     |
...
**定理10.1（三分质量分解）**：零点$\rho = 1/2 + i\gamma$对应的质量可分解为三个独立因子：

$$
m_\rho = m_\phi \cdot m_e \cdot m_\pi
$$
...
4. 三因子乘积满足归一化：
   $$
   m_\rho \approx m_0 \times \left(\frac{\gamma}{\gamma_1}\right)^{1/3} \times (0.403)^{1/2} \times e^{-0.194 \times 0} \times |\sin(\pi \times 0.403 \times 1)|^{1/3}
   $$
   在$\gamma = \gamma_1$时：
   $$
...
| $n$ | $\gamma_n$（前15位） | $(\gamma/\gamma_1)^{2/3}$ | $m_\phi/m_0$ | $m_e/m_0$ | $m_\pi/m_0$ | $m_\rho/m_0$ | Ω修正$\lambda_k^{1/2}$ |
|-----|---------------------|--------------------------|-------------|-----------|-------------|--------------|----------------------|
| 1   | 14.1347251...       | 1.0000                   | 0.6350      | 1.0000    | 0.8030      | 0.5100       | 1.2720               |
| 2   | 21.0220396...       | 1.3029                   | 0.7252      | 0.9736    | 0.7215      | 0.5098       | 1.2720               |
| 3   | 25.0108575...       | 1.4629                   | 0.7687      | 0.9628    | 0.6833      | 0.5057       | 1.2720               |
| 4   | 30.4248761...       | 1.6706                   | 0.8212      | 0.9489    | 0.6389      | 0.4979       | 1.2720               |
| 5   | 32.9350615...       | 1.7603                   | 0.8426      | 0.9422    | 0.6219      | 0.4937       | 1.2720               |
| 10  | 49.7738324...       | 2.3146                   | 0.9661      | 0.9012    | 0.5294      | 0.4609       | 1.2720               |
...
- 总质量$m_\rho = m_\phi \times m_e \times m_\pi$归一化至$m_0$
...
$$
S_{BH}^{\text{fractal}} = S_{BH} \times (1.055)^{1.789} \approx 1.101 \times S_{BH}
$$
修正率$+10.1\%$（精确值依赖$D_f^{\zeta}$的测量）。
