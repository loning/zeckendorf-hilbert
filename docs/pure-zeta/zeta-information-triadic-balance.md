...
$$
\frac{1}{\sqrt{3}} \leq |\vec{i}| \leq \frac{1}{\sqrt{2}}
$$

**证明**：
对于满足$i_+ + i_0 + i_- = 1$的向量，欧几里得范数$|\vec{i}|^2 = i_+^2 + i_0^2 + i_-^2$。

下界：当$i_+ = i_0 = i_- = 1/3$时，$|\vec{i}|^2 = 3 \cdot (1/3)^2 = 1/3$

上界：通过拉格朗日乘子法，在约束$i_+ + i_0 + i_- = 1$下最大化$i_+^2 + i_0^2 + i_-^2$，得到边界值。当$i_+ = 0, i_0 = 0.5, i_- = 0.5$时，$|\vec{i}|^2 = 0 + 0.25 + 0.25 = 0.5$
...
$$
\langle \vec{i}(1/2+it) \rangle_{t \to \infty} \approx \left(\frac{1}{2}, 0, \frac{1}{2}\right)
$$
...
**预言3**：临界系统的统计平均熵趋向$\log 2$

$$
\langle S_{\text{critical}} \rangle \to \log 2 \approx 0.693
$$
...
def compute_entropy(i_plus, i_zero, i_minus):
    """计算Shannon熵（仅当所有分量≥0时有定义）"""
    components = [i_plus, i_zero, i_minus]

    # 检查是否有负分量
    if any(p < 0 for p in components):
        return float('nan')  # 未定义
