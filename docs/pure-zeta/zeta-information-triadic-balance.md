...
**物理诠释**：
- $\mathcal{I}_+$：构造性信息，对应粒子态、局域化、确定性
- $\mathcal{I}_0$：波动性信息，对应波函数、非局域、概率幅
- $\mathcal{I}_-$：补偿性信息，对应真空涨落、虚粒子、负能态
...
**定义17.2（态矢量）**：
$$|\psi\rangle = i_+ |{\text{particle}}\rangle + i_0 |{\text{wave}}\rangle + i_- |{\text{field}}\rangle$$

**归一化**：
$$\langle\psi|\psi\rangle = i_+^2 + i_0^2 + i_-^2 = 1$$
...
def compute_triadic_components(s, N=10000):
    ...
    for n in range(1, N+1):
        ...
        if term.real > 0:
            i_plus += term.real * weight
        else:
            i_minus += abs(term.real) * weight

        i_zero += abs(term.imag) * weight
