...
宇宙的几何结构遵循同样的原理。时空不是被物质"压弯"的橡皮膜，而是信息在频域中分布不均的必然几何表现。每个频率都携带着曲率的种子，而负信息——那个神秘的-1/12——则是将这些种子编织成我们栖息的弯曲宇宙的隐秘力量。
...
对于幂律谱$P(\omega) \sim |\omega|^{-\alpha}$，当$\alpha > 2$时产生正曲率，由整体正规化的负信息$-1/12$平衡以维持总信息守恒$\mathcal{I}_{total} = 1$。
...
负信息$-1/12$源于整体双侧频谱的正规化：

$$\sum_{n=-\infty}^{\infty} |n| = -1/12$$

这个结果通过zeta函数的解析延拓$\zeta(-1) = -1/12$获得，确保总信息守恒$\mathcal{I}_+ + \mathcal{I}_- + \mathcal{I}_0 = 1$，其中负信息$\mathcal{I}_- = -1/12$补偿无限维系统的发散。
...
负信息-1/12不是数学技巧，而是宇宙存在的必要条件。它提供了平衡正曲率所需的负贡献，使得宇宙既不会立即坍缩，也不会立即撕裂。
...
负信息不是缺陷，而是特性。正如音乐需要休止符，绘画需要留白，宇宙需要负曲率来创造结构和意义。-1/12是宇宙的签名，写在每一个弯曲的时空褶皱里。
