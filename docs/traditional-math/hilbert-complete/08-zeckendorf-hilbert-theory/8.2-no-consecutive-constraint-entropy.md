...
#### 信息熵密度
$$h_{\text{No-11}} = \lim_{n \to \infty} \frac{\log |B_n|}{n} = \frac{\log \phi}{1} = \log \phi \approx 0.481 \text{ bits/symbol}$$

#### 约束代价
相对于无约束二进制：
$$\text{ConstraintCost} = \log 2 - \log \phi = \log \frac{2}{\phi} \approx 0.213 \text{ bits/symbol}$$
...
其中$\delta_{\text{No-11}} = \log \phi - \log(\phi-1) > 0$是No-11约束的熵增下界。
...
但对有限$n$：$\frac{F_{n+3}}{F_{n+2}} \geq \phi - \varepsilon_n$，其中$\varepsilon_n \to 0$

**步骤4**：严格熵增下界
$$\Delta S_n = \log \frac{F_{n+3}}{F_{n+2}} \geq \log(\phi - \varepsilon_n) > \log(\phi - \delta) = \delta_{\text{No-11}} > 0$$
...
\frac{\phi^k}{\sqrt{5} \cdot N_{\text{Zeck}}(n)} & \text{if } k \in \text{Zeckendorf}(n) \\
0 & \text{otherwise}
\end{cases}$$

其中$N_{\text{Zeck}}(n) = \sqrt{\sum_{k \in \text{Zeckendorf}(n)} \frac{\phi^{2k}}{5}}$是Zeckendorf归一化常数。
