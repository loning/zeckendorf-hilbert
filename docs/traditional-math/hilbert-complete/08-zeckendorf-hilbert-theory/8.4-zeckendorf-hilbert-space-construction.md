...
**递归生成**：
$$\mathcal{L}_{\text{Zeck}}^{(R)}(f_n) = f_n + \sum_{s \in B_{n+1} \setminus B_n} \phi^{-|s|} \langle f_n, \psi_s \rangle e_s$$

其中$\psi_s$是Zeckendorf生成函数。
...
**映射定义**：
$$\Psi(f_n) = \sum_{k=0}^n a_k \sum_{s: |s|=k, s \in B_n} \frac{e_s}{|s|!}$$

**逆映射**：
$$\Psi^{-1}(g) = \sum_{k=0}^n \left(\sum_{s: |s|=k} g_s |s|!\right) e_k$$
...
| 熵增不保证 | No-11律$\Rightarrow$严格熵增 | $\Delta S \geq \log \phi > 0$ |
