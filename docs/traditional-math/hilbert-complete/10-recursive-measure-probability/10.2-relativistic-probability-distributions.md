...
#### e模式概率分布
$$P_e^{(R)}(X = k) = \frac{e^{-2k/(k!)} |a_k|^2}{\sum_{j=0}^n e^{-2j/(j!)} |a_j|^2}$$
...
#### e模式收敛
$$\lim_{n \to \infty} P_{e,n}^{(R)}(X = k) = \frac{e^{-k} (1/k!)^2}{Z_e}$$
...
#### φ模式（Zeckendorf约束下）
收敛到修正的高斯分布：
$$Z_{\phi,n}^{(R)} \xrightarrow{d} \mathcal{N}_{\text{Zeck}}(0, \sigma_{\phi}^2)$$

其中$\sigma_{\phi}^2 = \frac{\ln \phi}{\ln 2}$与Zeckendorf熵密度相关。
...
#### π模式
收敛到对称分布：
$$Z_{\pi,n}^{(R)} \xrightarrow{d} \mathcal{S}_{\pi}(0, \sigma_{\pi}^2)$$

其中$\mathcal{S}_{\pi}$是π-对称分布。
...
**定理**：在适当条件下：
$$\frac{1}{n} \sum_{k=0}^n \eta^{(R)}(k; 0) X_k \xrightarrow{a.s.} \mathbb{E}[\eta^{(R)}(0; 0) X_0]$$

### 模式特定大数定律

#### φ模式（Zeckendorf修正）
$$\frac{1}{|B_n|} \sum_{k \in \text{Zeckendorf}(n)} \phi^k X_k \xrightarrow{a.s.} \mathbb{E}_{\text{Zeck}}[X]$$

#### e模式
$$\frac{1}{n} \sum_{k=0}^n \frac{X_k}{k!} \xrightarrow{a.s.} \frac{\mathbb{E}[X]}{e}$$

#### π模式
$$\frac{1}{n} \sum_{k=1}^n \frac{(-1)^{k-1}}{2k-1} X_k \xrightarrow{a.s.} \frac{\pi}{4} \mathbb{E}[X]$$
