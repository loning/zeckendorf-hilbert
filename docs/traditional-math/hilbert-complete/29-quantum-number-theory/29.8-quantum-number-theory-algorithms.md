...
**算法结构**：
1. **制备叠加态**：$|\psi_0\rangle = \frac{1}{\sqrt{M}} \sum_{a=1}^M |a\rangle$
2. **模指数运算**：$\hat{U}_{\text{mod}} |a\rangle |0\rangle = |a\rangle |a^r \bmod N\rangle$
3. **量子傅里叶变换**：$\text{QFT}|\psi\rangle$
4. **测量**：测量得到周期信息
