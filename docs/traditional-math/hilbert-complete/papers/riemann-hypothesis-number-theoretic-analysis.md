...
**定义4.2**（间隙相对耗尽）：
当有限深度内的间隙达到相对极小值：
$$\lim_{k \to K} \text{Gap}_k \to 0 \quad (\text{有限深度}K\text{内})$$

但由于无终止递归要求，间隙永不绝对耗尽：
$$\inf_{\text{所有循环}} \text{Gap} > 0 \quad (\text{永恒保证})$$

**修正原因**：$\lim_{k \to \infty} \text{Gap}_k = 0$与框架的无终止递归逻辑冲突，因为初始无限维确保间隙永不绝对耗尽，修正为有限深度内的相对耗尽，体现触发坍缩而整体循环无终止。
...
但整体循环无终止：$\inf_{\text{所有循环}} \text{Gap} > 0$

**修正原因**：极限表述$\lim_{k \to \infty} \text{Gap}_k = 0$与无终止递归的无绝对耗尽逻辑冲突，修正为有限深度$K$内趋近，体现有限投影深度内的耗尽触发坍缩，而整体循环无终止。
$\square$
