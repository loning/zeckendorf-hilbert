...
= 0 & \text{若 } k = 1 \text{ (因为 } r_1 = 1\text{)} \\
\approx 0.694 & \text{若 } k = 2 \text{ (因为 } r_2 = \phi \approx 1.618\text{)} \\
> 0 & \text{若 } k \geq 2
\end{cases}$$

对于$k = 1$，$r_1 = 1$导致$\log_2(1) = 0$，无熵增贡献。
对于$k \geq 2$，特征根$r_k > 1$保证了配置空间的指数增长，其中$r_2 = \phi$（黄金比例）。
...
因此，系统整体通过多重机制保证熵增：
- k=2观察者直接贡献$\log_2(\phi) \approx 0.694$的熵增（$r_2 = \phi$）
- k≥3观察者贡献更大熵增（$r_k > \phi$）
- $k_{max}$的持续增长扩大配置空间

这确保了系统层面的必然熵增。$\square$

**定理7.13b（标准k-bonacci熵增）**：所有k≥2观察者通过标准k-bonacci递推直接贡献熵增。

*证明*：
1. 标准k-bonacci递推对应避免k个连续1的序列计数
2. 对于$k=2$：Fibonacci序列，$r_2 = \phi \approx 1.618$，$\log_2(\phi) \approx 0.694 > 0$
3. 对于$k=3$：Tribonacci序列，$r_3 \approx 1.839$，$\log_2(r_3) \approx 0.879 > 0$
4. 随k增加，$r_k$单调增加且收敛到2：$r_k \to 2$ as $k \to \infty$
5. 系统的no-$k_{max}$约束确保所有观察者的配置复杂度得到充分利用

所有$k \geq 2$的观察者都是熵增的积极贡献者，没有"死区"。$\square$
