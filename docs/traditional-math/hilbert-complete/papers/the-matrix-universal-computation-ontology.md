...
因此，系统整体通过多重机制保证熵增：
- k=2观察者虽然$r_2=1$自身不产生熵增，但在Zeckendorf编码下为$\mathcal{O}_{max}$贡献熵增
- k≥3观察者直接贡献熵增（$r_k > 1$）
- $k_{max}$的持续增长扩大配置空间

这确保了系统层面的必然熵增。$\square$

**定理7.13b（Zeckendorf熵增贡献）**：k=2观察者通过Zeckendorf编码为系统贡献熵增。

*证明*：
1. 对于系统观察者$\mathcal{O}_{max}$，其k值为$k_{max} = |\bigcup_{i} I_{\mathcal{O}_i}|$
2. 当包含k=2观察者时，系统的Zeckendorf表示需要避免$k_{max}$个连续激活
3. 根据Zeckendorf定理，避免k个连续1的序列数为$F_{k+2}$（第k+2个Fibonacci数）
4. 对于大的$k_{max}$，这产生约$\phi^{k_{max}}$的配置数（$\phi$为黄金比例）
5. 因此即使k=2观察者的$r_2=1$，它们仍通过参与系统的no-$k_{max}$约束贡献熵增

k=2观察者不是熵增的"死区"，而是通过Zeckendorf编码参与系统整体的熵增。$\square$
