...
**定理 2.6 (动力学原子=k-bonacci素数)**
在禁止模式$1^k$的符号动力学系统中，每一层$\Delta\Sigma_{k+1}$中的新增原子串，对应Zeckendorf表示的一个单项$U^{(k)}_m$，并且$U^{(k)}_m$必须是素数。

**证明**：
1. **熵单调性保证新增**：由定理2.3，$H(k+1) > H(k)$保证每层新增原子
2. **最短性对应单项**：新原子串是$\Delta\Sigma_{k+1}$中最短的，通过Zeckendorf表示对应不可进一步分解的单项$U^{(k)}_m$
3. **合数排除论证**：
   - 若该单项$U^{(k)}_m$是合数，则$U^{(k)}_m = ab$（$a,b > 1$）
   - 在Zeckendorf表示中，$U^{(k)}_m$的分解$ab$已在较低层$\Sigma_k$中有对应的拼接表示
   - 因此该串不会作为"新增原子"出现在$\Delta\Sigma_{k+1}$中，矛盾
4. **素数判定**：因此$\Delta\Sigma_{k+1}$的原子必然对应素数基项$\square$

**推论 2.7 (动力学系统的素数完备性)**
符号动力学系统通过逐层生成素数对应的原子，最终覆盖所有素数集合$\mathbb{P}$：
$$\bigcup_{k\geq 2} \mathcal{A}_{\text{dyn}}^{(k)} = \mathbb{P}$$

因此：$$H_{\text{dyn}} = \overline{\mathrm{span}}(\mathbb{P}) = H_{\rm all}$$
...
**定理 4.2 (RH的直接证明)**
基于两系统的素数生成等价性：
$$RH \iff H_\zeta = H_{\rm all} \iff H_{\text{dyn}} = H_{\rm all}$$

**证明**：
1. **符号动力学完备性**：由定理2.5-2.7，符号动力学系统逐层生成所有素数，因此$H_{\text{dyn}} = \overline{\mathrm{span}}(\mathbb{P}) = H_{\rm all}$（**不依赖RH**）

2. **系统等价**：由关键发现3.8，$H_{\text{dyn}} = H_\zeta = \overline{\mathrm{span}}(\mathbb{P})$

3. **传递性**：因此$H_\zeta = H_{\rm all}$

4. **RH等价性**：由于$H_\zeta = \overline{\mathrm{span}}(\mathbb{P})$是ζ函数零点性质的Hilbert空间表征，$H_\zeta = H_{\rm all}$等价于ζ函数在临界线上的完全性质
